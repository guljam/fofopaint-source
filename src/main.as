package
{
    ...
    {
        private const APP_VERSION:Number = 27.00;
        private const APP_STATE_VERSION:Number = 2700
        private const TOOL_NONE:int = 0,
                      TOOL_PEN:int = (1 << 0),
                      TOOL_ERASE:int = (1 << 1),
                      TOOL_LINE:int = (1 << 2),
        ...
        private const REPLAY_IMAGE_CAHCHE_COMPLETE:int = (1 << 0),
                      REPLAY_IMAGE_CAHCHE_READY:int = (1 << 1),
                      REPLAY_IMAGE_CAHCHE_PROCESSING:int = (1 << 2);

        private const UPDATE_NONE:int = 0,
        ...
        private const CANVAS_MIN_SIZE:Number = 100
        ...
        private const BUTTON_OFF_ALPHA:Number = Math.round(0.25*256)/256,
                      ZERO_POINT:Point = new Point(0,0),
                      STAGE_BG_COLOR:uint = 0xCCCCCC;
        //파일 저장 경로 변수
        private const appStateFilePath:File = File.applicationStorageDirectory.resolvePath("appstate"+(APP_STATE_VERSION.toString())),
        ...
        //키 누름 관련 변수
        private var LAST_KEY:int = -1;//마지막 누른거 여기다가 저장 반복호출되는 keydown 함수에서 한번만 호출되게 하는변수
        private const KEY_BUFFER:Array = []; //정식 키 다운 눌러준 상태에서 다른 키가 눌러져 있으면 여기다가 저장
        private const COMMAND_CTRL:int = (1 << 0),
        ...
        //메뉴 요소
        private const stageBG:Sprite = new Sprite(), //드래그 불러오기가 stage공백에서는 안되서 수동으로 전체바탕으로 만들어줌
                      ...
                      mouseHint:mouseHintSet = new mouseHintSet(),
                      bottomHint:Object = cBottomHint(),
                      aboutPanel:aboutBox = new aboutBox(),
                      loadMenuBox:loadBox = new loadBox(),
                      controlBox:controlMenu = new controlMenu(),
                      colorPickerBox:colorPickerButtons = new colorPickerButtons(),
                      previewBox:previewPanel = new previewPanel(),
                      canvasInfoBox:canvasInfoBar = new canvasInfoBar(),
        ...
        //캔버스와 초창기 개발 변수들
        //펜툴 줌툴 미러 에어브러시 관련
        private var canvasBGColor:uint = 0xFFFFFF,
                    canvasWidth:Number = 600,
                    canvasHeight:Number = 390,
                    canvasLayer1BitmapData:BitmapData = new BitmapData(canvasWidth,canvasHeight,true,0),
                    canvasLayer2BitmapData:BitmapData = new BitmapData(canvasWidth,canvasHeight,true,0),
                    canvasDrawLayerBitmapData:BitmapData = new BitmapData(canvasWidth,canvasHeight,true,0),
                    ...
                    canvas2Bitmap:Bitmap = new Bitmap(canvasDrawLayerBitmapData,"auto",true),
                    penSizePreviewCursor:Shape = new Shape(), //펜사이즈 미리 보기
                    canvasAnchorPoint:Sprite = new Sprite(),//회전 스프라이트 부모
                    canvasPanel:Sprite = new Sprite(), //회색 부분을 제외한 그리기 영역 추가
                    canvasDrawLayer:Sprite = new Sprite(), //캔버스 2번 임시로 그려주는 캔버스 버퍼?
                    canvasDraw:Shape = new Shape(), //실제로 선을 긋는 요소
                    ...
                    canvasZoomedMultipler:Number = 1.0,
        ...
        //오른쪽 클릭 툴박스
        private var isToolBox2ON:Boolean = false; //툴박스가 오른쪽 클릭으로 켜졌을때 올려줌

        //undo 관련변수
        private var undoDataIndex:int = 0, //undo redo할때 무슨 이미지인지 알려주는 undoImageData의 포인터 인덱스임
        ...
        //lasso 관련 변수
        private var lassoMenuBox:lassoButtons = new lassoButtons(), //라소툴 버튼
                    ...
                    isLassoToolON:Boolean = false, //라소툴로 영역 선택하면 올려줌
        ...
        //save load 관련 변수
        private var isFileAlreadySaved:Boolean = false, //세이브 버튼 여러번 눌러서 데이터 계속 쓰여지는거 방지
        ...
        //키 오래누름 관련 변수
        private var pressHoldCountDownTime:Number = 0.0,
                    pressHoldFrameCount:int = 0

        //컬러 히스토리 관련 변수
        private var isMyPaletteExpended:Boolean = false, //전체로 보면 올려줌
        ...
        private var repFileTemp:File, //파일을 저장하거나 불러올때 씀
                    rFirstImageFile:File = replayCacheImageFolderPath.resolvePath("0"),
                    rFileStream:FileStream = new FileStream(), //함수들을 왔다갔다 해야해서 전역으로 하나
                    rCanvasLayer1BitmapData:BitmapData = new BitmapData(canvasWidth, canvasHeight, true, 0),
                    rCanvasLayer2BitmapData:BitmapData = new BitmapData(canvasWidth, canvasHeight, true, 0),
                    rCanvasDrawBitmapData:BitmapData = new BitmapData(canvasWidth, canvasHeight, true, 0),
                    ...
                    rReplayFOFOCursor:tinyCursor = new tinyCursor(); //재생할때 틀어주는 작은 마우스
변수 리펙토링 중이었음
                    ,rcanvas2ClipRect:Rectangle = new Rectangle() //갱신된 부분만 그려주는 거 오래된 버전 지원때문에 남겨둠
                    ,rcanvas2ClipRect2:Rectangle = new Rectangle() //갱신된 부분만 그려주는 거 이게 새거임
                    ,isReplayStarted:Boolean = false //리플레이 시작버튼 여러번 누르는거 방지
                    ,isReplayFinished:Boolean = true //리플레이가 자연히 끝났을때 올렽주는 플래그 가장 처음에 캔버스 싹쓸이 하기 위해서 넣어줌.
                    ...
                    ,rLastSubLayerFlag:Boolean = false, //리플레이 실행할때 이걸로 비교해서 캔버스 스왑해줌
        ...
        //캡쳐모드 관련 변수
        private var isCaptureModeON:Boolean = false //스크린샷 켜지면 올려줌
                    ...
                    ,captureStampFontListBox:capStampFontList = new capStampFontList()
        ...
        //윈도우 크기변수
                    ,lastAppWindowSize:Point = new Point() //창크기 조절 얼마나 됐을지 비교할때 마지막 크기 창크기 저장
        ...
        //참고 레이어 변수
        private const canvasRefLayer:Sprite = new Sprite()//트레이스 레이어임
        private var isRefLayerMemoryTrainingON:Boolean = false // 이거 켜지면 캔버스 그릴때 임시적으로 안보이게함
                    ,canvasRefLayerBitmapData:BitmapData = new BitmapData(canvasWidth,canvasHeight,true,0)
        ...
        private const realWorkingTimer:Object = cRealWorkingTimer()
                    ...
                    ,replayDrawCommands:Object = cReplayDrawCommands()
                    ,replayDraw:Function = cReplayDraw()
        ...
        //스크롤바 변수
        private var scrollSetMovedY:Number = 0
                    ,scrollBarHeight:Number = 0
                    ,sideBarConstHeight:Number = 780

        //ui 색깔 변수

        private var uiScaleIndex:int = 0
                    ,uiColorIndex:int = 1;
        private const uiScales:Array = [1.0,1.25,1.5,1.75,2.0,2.25]
                    ,uiColorSets:Array =
                    [                  
                                         //주 컬러,        주컬러 반대색,    stage배경색,  캔버스 조절 막대 색,   리플레이 완료 막대 색, 리플레이 재시작 막대색
        ...
        //새창 관련 변수
                    ...
                    ,isCanvasWindowON:Boolean = false //캔버스 새창 켜졌을때
                    ,canvasWindow:NativeWindow //참조된 새 창
                    ,canvasWindowLayer1Bitmap:Bitmap //새창안에 들어갈 레이어 1 2번
                    ,canvasWindowLayer2Bitmap:Bitmap
        ...
        // 딥언도 관련 변수
                    ,isDeepUndoEnabled:Boolean = false
                    ,lastDeepUndoEnabledFlag:Boolean = false //리플레이 켜줄때 딥 플래그를 꺼줘서 여기다가 미리 저장해둠
                    ,deepUndoStartFrame:Number = -1 //리플레이 켜줄때 rNowFrame이 변하니까 그전에 백업해주고 꺼주고 다시 undo실행할때 이 프레임 기준으로 하려고

        //picker box RGB info관련 변수
                    ,isRgbInfoTextFocused:Boolean = false // rgb info입력이 활성화 되었을때 올려줌
                    ,selectedRGBInfoTextIndex:int = -1 //처음 클릭했을때 R G B중 어느 영역을 클릭했는지
                    ,lastRgbInfoTextCursorIndex:int = -1 //포커스 아웃 될때 마지막 커서 위치가 어딘지 저장
                    ,isIgnoringRgbInfoTextRightClick:Boolean = false // RGB INFO 오를쪽 클릭은 힌트 안뜨고 기능못하게함
                    ,isRgbInfoTextHSV:Boolean = false // true가 되면 hsv false이면 rgb

        //기타
        private var isWindowClosing:Boolean = false//윈도우 닫힐때 올려줌 save all data가 windows closing일때는 무조건 해주게 끔함
                    ,lastWindowDeactivateTime:int = 0 //윈도우 비활성화된 시간 저장, 너무 자주 알탭해서 save all data가 자주 호출되는걸 막음
                    ,isPenSizeCursorInvisible:Boolean = false //펜커서 이게 on되면 안보여줌
                    ,invokeDragDropFile:File //invoke 이벤트에서 파일을 너무 빨리 불러오지 못하게함
                    ,loadMenuBoxPreviewImageData:BitmapData
                    ,loadMenuBoxFile:File
                    ,lastEraserPosButton:SimpleButton = null //툴 선택해줬을때 지우개툴이 이동한 툴을 저장해줌 다시원래대로 복원해주려고
                    ,isCanvasZoomHintVisible:Boolean = false //툴박스에서 마우스 클릭해서 줌툴써줄때 mouse out이벤트가 가장 늦게 되서 줌 배율 힌트가 처음에 보이지 않는거 해결
                    ,isRightSidebar:Boolean = false // 사이드바 위치 0이 왼쩾 1이 오른쪽
                    ,isSidebarVisible:Boolean = true
                    // ,sideBarPosSave:Number //사이드 바 단축키 사용하고나서 원래 위치로 옮겨줄때 씀
                    ,isQuickSidebarActive:Boolean = false
                    ,isUpdatePendingAfterSaving:Boolean = false //업데이트 버튼 눌렀을때 파일 저장 해주고 기다려주는 플래그
                    ,isLoadPendingAfterSaving:Boolean = false //파일을 로드해주는데 파일 세이브 끝나고 로드 해주는 플래그
                    ,isLayerCheckKeyPressed:Boolean = false //w키 1키 계속 누르고 있을때 함수 호출 안하게 해주려고 플래그 올려줌
                    ,isDrawModeInputEventsAdded:Boolean = false // 이벤트 세트가 켜지거나 꺼지는거 보관, 중복 이벤트 추가 피하려고
                    ,isReplayModeInputEventsAdded:Boolean = false // 이벤트 세트가 켜지거나 꺼지는거 보관, 중복 이벤트 추가 피하려고
                    ,isFileBrowserOpened:Boolean = false //캡쳐 저장키 빠르게 누를때 에러 떠서 중복안되게 플래그 세워줌
                    ,legacyReplaytotalFrameSave:Number = -1; //24.00버전 이후로 쓸일 없지만 이전버전 호환성을 위해서 백업해주고 복원해줌
        ...
        private function letThereBeLight():void
        {
            ...
            addStageInputEvents();
            addInputEventsStageChild();
            addInputEventsDrawMode();
            initializeReplayDataFile();
            initializeColorPickerBoxInfo(penColor);
            centerCanvas();
            centerCanvas(true);
            previewBox.updateImage(canvasLayer1BitmapData,canvasLayer2BitmapData,canvasBGColor);
            realWorkingTimer.start();
            checkForUpdates();
            tryDisableIME();
            selectPenTool();
            colorPickerBox.setActiveColorPreset(0);
            bottomHint.updateScale(getUIScale());
            bottomHint.setCursorColor(hintHighlightBoxColors[uiColorIndex]);
        ...
        //function
        private function calculateSliderValueFromMouseX(mousex:Number,minx:Number,maxx:Number,minvalue:Number,maxvalue:Number,cursor:DisplayObject):Number
        {
            if(mousex < minx)
            {
                mousex = minx;
        ...
        private function startDragInteraction(params:Object,onDragStart:Function,onMouseMove:Function,onMouseUp:Function):void
        {
            isMouseDragging = true;

            function mouseMouseUpDrag(e:MouseEvent):void
            {
                isMouseDragging = false;
                stage.removeEventListener(MouseEvent.MOUSE_UP,mouseMouseUpDrag);
            ...
            stage.addEventListener(MouseEvent.MOUSE_UP,mouseMouseUpDrag);
        }
        ...
        private function disableTransparentBGDrawMode():void
        {
        ...
        private function enableTransparentBGDrawMode():void
        {
            if(!canvasPanel.getChildByName("canvasFlash"))
            {
                ...
                canvasFlashEffect.graphics.drawRect(0,0,canvasWidth,canvasHeight);
        ...
        private function executeCaptureFlashEffect():void
        {
            if(drawCaptureArea.isFullImageCapture())
            {
                ...
                {
                    applyCanvasFlashEffect(canvasPanel,0,0,canvasWidth,canvasHeight);
                }
            }
            else
            {
        ...
        private function applyCanvasFlashEffect(parent:DisplayObjectContainer,ox:Number,oy:Number,width:Number,height:Number):void
        {
        ...
        private function selectOrResetMyPalette():void
        {
        ...
        private function startSelectOrAddColorMyPalette():void
        {
        ...
        private function isMouseCursorInSideBar():Boolean
        {
        ...
        private function onMouseWheelStage(e:MouseEvent):void
        {
            ...
            || isPopUpWindowOpened()
            || isCaptureModeON || !isQuickSidebarActive && isKeyPressed() || getCommandKey() !== 0) return;
            ...
            if(!hasTimer("wheelZoomTimer"))
            {
                addTimerByName("wheelZoomTimer",0.07,false,function():void
                {
                    ...
                    else if(isCursorInDrawArea())
                    {
                        if(e.delta > 0)
                        {
                            setZoomInButton(true,isReplayModeON);
                            if(!isReplayModeON)
                            {
                                showMouseHintTemporary(Math.floor(canvasZoomedMultipler*100)+"%",1.5);
                            }
                        }
                        else
                        {
                            setZoomInButton(false,isReplayModeON);
                            if(!isReplayModeON)
                            {
                                showMouseHintTemporary(Math.floor(canvasZoomedMultipler*100)+"%",1.5);
                            }
        ...
        {
            var startIndex:int = colorPickerBox.rgbInfo.selectionBeginIndex;
            var endIndex:int = colorPickerBox.rgbInfo.selectionEndIndex;
            var cursorIndex:int;
            ...
            if(isRgbInfoTextHSV)
            {
                if(num === "0" && int(getRGBColorTextFromRGBInfoText()[lastRgbInfoTextCursorIndex]) === 0)
                {
                    return;
                }
            }
            else if(num === "0" && int(getRGBColorTextFromRGBInfoText()[lastRgbInfoTextCursorIndex]) === 0)
            {
                return;
            }
            ...
            {
                colorPickerBox.rgbInfo.replaceText(startIndex,endIndex,num);
            }
            else
            {
                colorPickerBox.rgbInfo.replaceText(startIndex,startIndex,num);
            }
        ...
        private function startPressHoldKey(button:DisplayObject,hintStr:String,readyFunc:Function,okFunc:Function,cancelFunc:Function):void
        {
            if(!hasTimer("pressholdtimer"))
            {
                ...
                const countDownTimeNow:Number = Math.ceil((stage.frameRate*3)/countDownTime);
        ...
        private function initializeRepTempFile():void
        {
            repFileTemp = File.applicationStorageDirectory.resolvePath("tmp\\tmp_"+getRandomString(32));
        }

        private function executeLoadMenuBoxClick(oldTargetName:String):void
        {
        ...
        private function onMouseDownLoadMenuBox(e:MouseEvent):void
        {
            if(!e.target)
            {
                return;
        ...
        private function setLoadBoxOFFLoadFailed():void
        {
            ...
            showMouseHintTemporary("Load failed");
            mouseHint.x = Math.floor(loadMenuBox.x);
            mouseHint.y = Math.floor(loadMenuBox.y);
            setLoadBoxVisible(false);
        }

        private function activeColorPreset(type:int):void
        {
        ...
        private function addInputEventsDrawModeOrReplayMode():void
        {
            if(isReplayModeON)
            {
        ...
        private function resetRCanvasDrawLayerCliprect2():void
        {
        ...
        private function updateRCanvasDrawLayerCliprect2():void
        {
            rcanvas2ClipRect2 = rcanvas2ClipRect2.union(rCanvasDraw.getBounds(rCanvasPanel));
        }
        ...
        private function resetRCanvasDrawLayerCliprect():void
        {
        ...
        private function updateRCanvasDrawLayerCliprect():void
        {
            rcanvas2ClipRect = rcanvas2ClipRect.union(rCanvasDraw.getBounds(rCanvasPanel));
        }

        private function resetCanvasDrawLayerCliprect():void
        {
        ...
        private function extandCanvasDrawLayerCliprect():void
        {
        ...
        private function updateCanvasDrawLayerCliprect():void
        {
            canvasDrawLayerClipRect = canvasDrawLayerClipRect.union(canvasDraw.getBounds(canvasPanel));
        }

        private function normalizeAlphaValue(alp:Number):Number
        {
            //객체의 alpha값이 8비트int로 변환된후 다시 Number로 변환되기 때문에 실제 소수점 비교를 할때도 같은 방식을 써주어야함
            return Math.round(alp*256)/256;
        }

        private function getCanvasLayerSwappedHintString():String
        {
            return "Layers has been swapped "+((isLayerSwapped) ? "1 / 2" : "2 / 1");
        }

        private function createPosUpdaterByMouseDrag(target:DisplayObject,targetAngle:Number,customScaleX:Number=1.0,customScaleY:Number=1.0):Function
        {
            var oldX:Number = target.x;
            var oldY:Number = target.y;
            var mx:Number = mouseX;
            var my:Number = mouseY;
            const zoom:Number = canvasZoomedMultipler;
        ...
        private function hideCanvasRotateCursor():void
        {
            canvasRotateCursor.visible = false;
        }

        private function createAngleUpdaterByMouseDrag(target:DisplayObject):Function
        {
        ...
        private function createScaleUpdaterFromMouseDrag(sc:Number):Function
        {
        ...
        {
            const strColor:String = getColorInfoStringOfHex(colorPickerBox.getCurrentColor(),isRgbInfoTextHSV);

            return "Current color : "+strColor;
        }

        private function isHintUnavailable():Boolean
        {
            return isMouseClicked || isRightMouseClicked || isMouseDragging || isToolBox2ON || isFillPenStarted
            || isLassoToolON || isRgbInfoTextFocused || isAboutWindowShowing || rReplayImageCacheState === REPLAY_IMAGE_CAHCHE_PROCESSING
            || isPopUpWindowOpened();
        }

        private function playLayerSwapFlickEffect(target:DisplayObject):void
        {
        ...
        private function onMouseOverPickerBoxHintON(e:MouseEvent):void
        {
            const target:DisplayObject = e.target as DisplayObject;
            if(!target || colorPickerBox.alpha < 1.0) return;
            const targetName:String = target.name;
            var str:String = "";

            if(isHintUnavailable() && !isFillPenStarted)
            {
                return;
            }
            ...
            switch(targetName)
            {
                ...
                case "rgbInfo":
                {
                    ...
                    str = "Change value [click]\nChange color model [click "+((isRgbInfoTextHSV) ? "'HSV'":"'RGB'")+" text]";
                }
                break;
            ...
            bottomHint.show(str,target);
        }

        private function onMouseOverSideBarScrollHintON(e:MouseEvent):void
        {
            const target:DisplayObject = e.target as DisplayObject;
            if(!target || (isHintUnavailable() && !isFillPenStarted))
            {
                return;
            }
        ...
        private function onMouseOutGlobalHintOFF(e:MouseEvent):void
        {
            ...
            if(!isRgbInfoTextFocused && !isMouseDragging && !isCaptureModeON)
            {
                if(mouseHint.isShowing() || bottomHint.isHighlightBoxShowing())
                {
                    bottomHint.hide();
                }
            }
            ...
            {
                if(mouseHint.hitTestPoint(mouseX,mouseY))
                {
                    if(mouseHint.y !== 0)
                    {
                        mouseHint.y = 0;
        ...
        {
            bottomHint.show(str,null,true);
            sethintOFFDelay();
        }

        private function cBottomHint():Object
        {
            const hintHighlightBox:Shape = new Shape() //요소에 마우스 클릭하면 사각형으로 하이라이트 표시해줌
            var gp:Point = new Point(0,0);
            var scale:Number = 1.0;
            var w:Number = 0;
            var h:Number = 0;
            var lastHintStr:String;
            ...
            function hideHighlightBox():void
            {
                hintHighlightBox.visible = false;
            }
            
            function isHighlightBoxShowing():Boolean
            {
                return hintHighlightBox.visible;
            }

            function showHighlightBox():void
            {
                hintHighlightBox.visible = true;
            }

            function getHighlightBox():Shape
            {
                return hintHighlightBox;
            }

            function checkMoveingHint(e:Event):void
            {
                if(mouseHint.width > stage.stageWidth)
                {
                    if(hintMoveWaitCountFirst >= 1000)
                    {
                        if(hintMoveDirection)
                        {
                            if(mouseHint.x+mouseHint.width > stage.stageWidth)
                            {
                                mouseHint.x = mouseHint.x-hintMoveSpeed*mouseHint.scaleX;
                            }
                            else
                            {
            ...
            function showHintHighlightBox(target:DisplayObject):void
            {
                ...
                hintHighlightBox.graphics.lineStyle(2.0*scale,cursorColor);

                if(target is TextField)
                {
                    hintHighlightBox.x = gp.x-2*scale;
                    hintHighlightBox.y = gp.y-3*scale;
                }
                else if(target === controlBox.airBrushButtonWrapper || target === controlBox.sharpLineButtonWrapper)
                {
                    hintHighlightBox.x = gp.x-scale;
                    hintHighlightBox.y = gp.y-scale;
                }
                else
                {
                    hintHighlightBox.x = gp.x;
                    hintHighlightBox.y = gp.y;
                }

                hintHighlightBox.graphics.drawRect(0,0,w,(target is TextField) ? h-scale:h);

                setAsTopChild(hintHighlightBox);
                hintHighlightBox.visible = true;
            }

            function hintHintImmediately():void
            {
                hintHighlightBox.visible = false;
                lastHintStr = null;
                targetSave = null;
            ...
            function hintFullOFF():void
            {
                ...
                hintONTime = 0;
                mouseHint.hide();
                mouseHint.setHintText("");
                mouseHint.x = -mouseHint.width-3;
                mouseHint.y = -mouseHint.height-3;
            }

            function updateHintPos():void
            {
                mouseHint.x = 0;
                mouseHint.y = Math.round(stage.stageHeight-mouseHint.getScaledHeight()+1);
            }

            function hintON(str:String,target:DisplayObject,fastHint:Boolean=false):void
            {
                if(!hintHighlightBox.visible && !fastHint)
                {
                    return;
                }
            ...
            function showHint(str:String,target:DisplayObject,fastHint:Boolean=false):void
            {
                str = str.replace(/\n/g, " _ ");
                if(mouseHint.isShowing() && lastHintStr === str)
                {
                    return;
                }
                ...
                if(!target || fastHint || getTimer()-hintONTime < 1000 || mouseHint.isShowing())
                {
            ...
            return {
                ...
                showHintHighlightBox:showHintHighlightBox,
                updateScale:updateScale,
                setCursorColor:setCursorColor,
                isHintShowing:isHintShowing,
                hintHintImmediately:hintHintImmediately,
                getHighlightBox:getHighlightBox,
                isHighlightBoxShowing:isHighlightBoxShowing,
                showHighlightBox:showHighlightBox,
                hideHighlightBox:hideHighlightBox
            }
        }

        private function isRGBInfoValueChanged():Boolean
        {
            return colorPickerBox.getOldRGBInfoText() !== colorPickerBox.getFirstRGBInfoColorText();
        }

        private function checkRGBInfoHasEmptyValue():Boolean
        {
            const c:Array = getRGBColorTextFromRGBInfoText();
            var rgbEmptyIndex:int = -1;
            ...
            //이전 항목으로 이동
            --rgbEmptyIndex;
            if(rgbEmptyIndex < 0) rgbEmptyIndex = 0;
            selectRGBInfoTextByRGBPos(rgbEmptyIndex);
        ...
        {
            const textBeforeCursor:String = colorPickerBox.getRGBInfoText().substring(0,colorPickerBox.rgbInfo.caretIndex);
        ...
        private function selectRGBInfoTextByRGBPos(index:int):void
        {
            ...
            if(index === 0)
            {
                colorPickerBox.rgbInfo.maxChars = 15;
                colorPickerBox.rgbInfo.setSelection(4,colorPickerBox.getRGBInfoText().indexOf(","));
            }
            else if(index === 1)
            {
                colorPickerBox.rgbInfo.maxChars = 15;
                colorPickerBox.rgbInfo.setSelection(colorPickerBox.getRGBInfoText().indexOf(",")+1,colorPickerBox.getRGBInfoText().lastIndexOf(","));
            }
            else if(index === 2)
            {
                colorPickerBox.rgbInfo.maxChars = getRGBInfoTextLimit();
                colorPickerBox.rgbInfo.setSelection(colorPickerBox.getRGBInfoText().lastIndexOf(",")+1,colorPickerBox.getRGBInfoText().length);
            }

            selectedRGBInfoTextIndex = index;
        }
        ...
        private function checkRGBInfoColorValueLimit():void
        {
            ...
            if(isRgbInfoTextHSV)
            {
                ...
                colorPickerBox.setRGBInfoText("HSV "+c[0]+","+c[1]+","+c[2]);
            }
            else
            {
                ...
                colorPickerBox.setRGBInfoText("RGB "+c[0]+","+c[1]+","+c[2]);
            }
        }

        private function moveRGBInfoTextCursor(value:int):void
        {
            const cursorPos:int = colorPickerBox.rgbInfo.caretIndex;
            if(value === 1)
            {
                colorPickerBox.rgbInfo.setSelection(cursorPos+1,cursorPos+1);
            }
            else
            {
                colorPickerBox.rgbInfo.setSelection(cursorPos-1,cursorPos-1);
        ...
        {
            const index:int = (fromNumPad) ? lastRgbInfoTextCursorIndex:getRGBInfoTextCursorPos();
            ...
            colorPickerBox.setRGBInfoText("HSV "+hsv);
            colorPickerBox.updateOldRGBInfoText();
        ...
        private function updateRGBInfoTextColorFromNumpad(value:int,fromNumPad:Boolean):void
        {
            const index:int = (fromNumPad) ? lastRgbInfoTextCursorIndex:getRGBInfoTextCursorPos();
            const rgb:Array = getRGBColorTextFromRGBInfoText();
            ...
            colorPickerBox.setRGBInfoText("RGB "+rgb);
            colorPickerBox.updateOldRGBInfoText();
        ...
        {
            if(colorPickerBox.rgbInfo.hitTestPoint(mouseX,mouseY))
            {
                var clickedPos:int = colorPickerBox.rgbInfo.getCharIndexAtPoint(colorPickerBox.rgbInfo.mouseX,10);
        ...
        private function showNumPad():void
        {
            ...
            {
                numPadBox.on(HSVtoHEX(hsvColorData[0],1.0,1.0),colorPickerBox.getRGBInfoBGColor(),getInvertedColor);
                setAsTopChild(numPadBox);
                const gp:Point = colorPickerBox.rgbInfoBG.localToGlobal(ZERO_POINT);
                numPadBox.x = gp.x;
                numPadBox.y = gp.y+colorPickerBox.rgbInfoBG.height*getUIScale();
                resetLastKey();
        ...
        private function numPadRightMouseDownEvent(e:MouseEvent):void
        {
            if(!e.target) return;
            setStageFocusNull();
            isIgnoringRgbInfoTextRightClick = true;
        }
        ...
        private function numPadMouseDownEvent(e:MouseEvent):void
        {
            ...
            if(!numPadBox.hitTestPoint(mouseX,mouseY) && !colorPickerBox.rgbInfo.hitTestPoint(mouseX,mouseY))
            {
        ...
        private function toggleRGBInfoTextColorType():void
        {
            ...
            if(isRgbInfoTextHSV)
            {
                isRgbInfoTextHSV = false;
                colorPickerBox.setRGBInfoText(getRGBInfoString(colorPickerBox.getRGBInfoBGColor()));
            }
            else
            {
                isRgbInfoTextHSV = true;
                colorPickerBox.setRGBInfoText(getHSVInfoString(HEXtoHSV(colorPickerBox.getRGBInfoBGColor())));
            }
        ...
        private function setRGBInfoTextInputOK():void
        {
            ...
            else if(isBackgroundColorMode())
            {
                ...
                if(isCanvasWindowON) updateCanvasWindowBGColor(canvasBGColor,canvasWindowLayer1Bitmap.bitmapData);
        ...
        private function rgbInfoTextFocusOutEvent(e:FocusEvent):void
        {
            ...
            lastRgbInfoTextCursorIndex = selectedRGBInfoTextIndex;

            if(isIgnoringRgbInfoTextRightClick)
            {
                isIgnoringRgbInfoTextRightClick = false;
                colorPickerBox.rgbInfo.selectable = true;
                return;
            }

            // stagefo();
            bottomHint.hide();
            colorPickerBox.rgbInfo.background = false;
            colorPickerBox.rgbInfo.border  = false;
            colorPickerBox.rgbInfo.removeEventListener(Event.ENTER_FRAME,checkRGBInfoCursorPos);
            ...
            colorPickerBox.rgbInfo.removeEventListener(Event.CHANGE, onRGBInfoTextChangeEvent);
            isIgnoringRgbInfoTextRightClick = false;
        ...
        {
            if(isIgnoringRgbInfoTextRightClick || isLassoToolON)
            {
                //라소툴때문에 강제로 올려줌
                isIgnoringRgbInfoTextRightClick = true;

                colorPickerBox.rgbInfo.selectable = false;
                colorPickerBox.updateOldRGBInfoText();
            ...
            var currnetTextCursorPos:int = colorPickerBox.rgbInfo.getCharIndexAtPoint(colorPickerBox.rgbInfo.mouseX,10);
            if(isRgbInfoTextFocused === false && isHSVRGBText && currnetTextCursorPos >= 0 && currnetTextCursorPos <= 3)
            {
                addTimerByName("textInputFocusIgnoreDelay",0.0,false,function():void
                {
                    setStageFocusNull();
            ...
            isRgbInfoTextFocused = true;
            isTransparentPenColor = false;
            selectedRGBInfoTextIndex = -1;
            ...
            else //투명색으로 transparent 텍스트가 되있을때 말하는거임
            {
                ...
                colorPickerBox.setRGBInfoText(colorPickerBox.getOldRGBInfoText());
            }
            ...
            colorPickerBox.rgbInfo.addEventListener(Event.CHANGE,onRGBInfoTextChangeEvent);
            ...
            addTimerByName("rgbInfoTextFocusInEventDelayCheck",0.0,false,function():void
            {
                ...
                colorPickerBox.rgbInfo.setSelection(currnetTextCursorPos,currnetTextCursorPos);
                colorPickerBox.rgbInfo.addEventListener(Event.ENTER_FRAME,checkRGBInfoCursorPos);
                bottomHint.show(STRING_CUSTOM_COLOR_HINT,colorPickerBox.rgbInfo);
            });
        }
        ...
        private function checkRGBInfoTextFormat(fromNumPad:Boolean):void
        {
            ...
            var isMatch:Boolean = pattern.test(colorPickerBox.getRGBInfoText());
            ...
            {
                colorPickerBox.resetToOldRGBInfoText();
            }
        }
        ...
        {
            if(colorPickerBox.rgbInfo.caretIndex < 4)
            {
                colorPickerBox.rgbInfo.setSelection(4,4);
            }

            if(colorPickerBox.rgbInfo.caretIndex < 0)
            {
                selectRGBInfoTextByRGBPos(2);
            }
            else if(selectedRGBInfoTextIndex !== getRGBInfoTextCursorPos())
            {
        ...
        private function setRCursorMirrorPos():void
        {
            ...
            const half:Number = canvasWidth/2;
        ...
        private function checkFOFOSideBarCollision():int
        {
            ...
            const gp:Point = sideBarScrollPanel.localToGlobal(ZERO_POINT);
        ...
        private function updateCanvasWindowBGColor(color:uint,bmpd:BitmapData):void
        {
        ...
        {
            const bounds:Rectangle = previewBox.setFitBitmapforBox(canvasLayer1BitmapData.width,canvasLayer1BitmapData.height
                                                                  ,canvasWindow.stage.stageWidth,canvasWindow.stage.stageHeight);
            updateCanvasWindowBGColor(canvasBGColor,canvasLayer1BitmapData);
        ...
        private function onMoveCanvasWindow(e:Event):void
        {
            updateCanvasWindowData();
        }

        private function onResizeCanvasWindow(e:Event):void
        {
        ...
        {
            canvasWindowLayer1Bitmap.bitmapData = previewBox.prevBitmap.bitmapData;
            canvasWindowLayer2Bitmap.bitmapData = previewBox.prevBitmapSub.bitmapData;
        ...
        private function onMouseDownCanvasWindow(e:MouseEvent):void
        {
            canvasWindow.startMove();
        }

        private function onRightMouseUpCanvasWindow(e:MouseEvent):void
        {
        ...
        private function onKeyDownCanvasWindow(e:KeyboardEvent):void
        {
            if(e.keyCode === KEY.esc) closeCanvasWindowTemp();
        }

        private function onClosingCanvasWindow(e:Event):void
        {
        ...
        private function initializeCanvasWindow():void
        {
        ...
        private function onActivateCanvasWindow(e:Event):void
        {
        ...
        {
            var layer1FlagStr:String = ((canvasLayer1Bitmap.visible)?"ON":"OFF");
            var layer2FlagStr:String = ((canvasLayer2Bitmap.visible)?"ON":"OFF");
            ...
            showMouseHintTemporary(layer1Str+"\n"+layer2Str);
        }
        ...
        private function enableNewFileButton():void
        {
            if(!isInSaveProgress && topBar.newFileButton.alpha < 1.0) topBar.newFileButton.alpha = 1.0;
        ...
        private function isAllLayerInvisible():Boolean
        {
            ...
            {
                showMouseHintTemporary("All layer locked");
                return true;
            }
            return false;
        }
        ...
        private function mergeLassoImageToRefLayer():void
        {
            ...
            {
                if(isDeepUndoEnabled) setApplyDeepUndo();
        ...
        {
            var bmpd:BitmapData = new BitmapData(canvasWidth,canvasHeight,true,0);
            const mat:Matrix = new Matrix();

            mat.scale(canvasRefLayer.scaleX,canvasRefLayer.scaleY);
            mat.rotate(canvasRefLayer.rotation*Math.PI/180)
            mat.translate(canvasWidth/2,canvasHeight/2);
        ...
        private function setLayer1CheckToggleCaptureMode():void
        {
            ...
            if(isReplayModeON)
            {
                if(rCanvasLayer1Bitmap.visible)
                {
                    rCanvasLayer1Bitmap.visible = false;
                    if(!isSubLayerONReplayMode()) rCanvasDrawLayer.visible = false;
                ...
                else
                {
                    ...
                    if(!isSubLayerONReplayMode()) rCanvasDrawLayer.visible = true;
        ...
        private function setLayer2CheckToggleCaptureMode():void
        {
            ...
            if(isReplayModeON)
            {
                if(rCanvasLayer2Bitmap.visible)
                {
                    rCanvasLayer2Bitmap.visible = false;
                    if(isSubLayerONReplayMode()) rCanvasDrawLayer.visible = false;
        ...
        private function addUndoBGColorData(color:uint):void
        {
        ...
        private function setLayerMergeButton():void
        {
            ...
            else
            {
                ...
                canvasLayer1BitmapData.fillRect(new Rectangle(0,0,canvasWidth,canvasHeight),0);
        ...
        private function setLayerSwapButton():void
        {
            ...
            playLayerSwapFlickEffect(controlBox.layerSwapButton);
        }
        ...
        private function getCaptrueImageBitmapdata(clipBoardCopyFlag:Boolean):BitmapData
        {
            ...
            if(isCaptureStampEnabled && tmpbmpd.width >= 300) drawCaptureStamp.kungFinal(tmpbmpd);
            bmpd.dispose();
        ...
        {
            setUIScaleButton(uiScales.length);
        }

        private function setUIScaleButton(index:int):void
        {
            if(index > uiScales.length-1) index = 0;
            uiScaleIndex = index;
        ...
        private function _closeQuickSidebar():void
        {
            ...
            checkFOFOPosition();
            colorPickerBox.rgbInfo.type = TextFieldType.INPUT;
        ...
        private function closeQuickSideBar():void
        {
        ...
        {
            if(!e.target || isRgbInfoTextFocused || isPopUpWindowOpened())
            {
                return;
            }
            ...
            switch(e.target.name)
            {
                ...
                case "zoomOutButton":
                {
                    if(canvasZoomedMultipler !== 1.0) resetZoomDrawMode();
                }
                break;
            ...
            if(colorPickerBox.rgbInfo.type === TextFieldType.INPUT)
            {
                closeQuickSideBar();
            }
        }
        ...
        private function setQuickSidebarON(shortcut:Boolean):void
        {
            ...
            if(shortcut)
            {
                colorPickerBox.rgbInfo.type = TextFieldType.DYNAMIC;
                setNowToolByOldTool();
        ...
        private function updateLastKey(key:int):void
        {
            LAST_KEY = getLastKey();
        }

        private function resetLastKey():void
        {
            LAST_KEY = -1;
        }

        private function isLastKey(key:uint):Boolean
        {
            return LAST_KEY === key;
        }
        ...
        private function checkPenOptionsKeyDown(keyCode:uint):Boolean
        {
        ...
        private function isToolActive():Boolean
        {
            ...
            showMouseHintTemporary("Tool locked");
        ...
        private function isCurrentLayerActive():Boolean
        {
            ...
            if(isSubLayerON === false)
            {
                ...
                {
                    showMouseHintTemporary("Layer 1 locked");
                    return false;
                }
            }
            else if(isSubLayerON === true)
            {
                if(canvasLayer2Bitmap.visible)
                {
                    return true;
                ...
                {
                    showMouseHintTemporary("Layer 2 locked");
                    return false;
                }
            }
            return false;
        ...
        private function initializeStageSettings():void
        {
        ...
        private function removeInputEventsLassoTool():void
        {
        ...
        private function addInputEventsLassoTool():void
        {
        ...
        private function onMouseDownStage(e:MouseEvent):void
        {
            ...
            if(bottomHint.isHintShowing() !== null)
            {
                bottomHint.hintHintImmediately();
            }
        }

        private function onRightMouseDownStage(e:MouseEvent):void
        {
            checkInvalidKey();
        ...
        private function onMiddleMouseDownStage(e:MouseEvent):void
        {
        ...
        private function onMouseUpStage(e:MouseEvent):void
        {
            ...
            if(!isMouseClicked && isRightMouseClicked) isMouseDragging = false;
        ...
        private function onRightMouseUpStage(e:MouseEvent):void
        {
            checkInvalidKey();
            const mx:Number = mouseX;
            const my:Number = mouseY;
        ...
        {
            if(isReplayModeON || isCaptureModeON || toolBox2.visible)
            {
                return;
            }
        ...
        private function markWindowTitleAsDirty():void
        {
            ...
            if(titleEndStr > 0 && stage.nativeWindow.title.charAt(titleEndStr-1) !== "*")
            {
                ...
                if(isCanvasWindowON) setSyncWindowTitle();
            }
        }
        ...
        private function selectTransparentColorButton():void
        {
            isTransparentPenColor = true;
            colorPickerBox.updateOldRGBInfoText();
            colorPickerBox.setRGBInfoBackgroundTransparent(myPalettePresetType);
            colorPickerBox.setRGBInfoText("Transparent");
        }

        private function updateCurrentColorPickerBox(bgmode:Boolean):void
        {
            const hexColor:uint = colorPickerBox.currentColorColor;
            ...
            if(bgmode)
            {
                ...
                if(isCanvasWindowON) updateCanvasWindowBGColor(canvasBGColor,canvasWindowLayer1Bitmap.bitmapData);
                moveHSVCursorByColor((isRgbInfoTextHSV) ? HEXtoHSV(hexColor) : hexColor);
        ...
        private function cFillPenTool():Object
        {
            ...
            function updateFillPenPreview(e:Event):void
            {
                ...
                if(!sideBar.visible || !sideBar.hitTestPoint(mouseX,mouseY) && !isMouseClicked)
                {
            ...
            {
                if(canvasSizeRect.intersects(canvasDraw.getBounds(canvasPanel)))
                {
                    return true;
                }
                return false;
            ...
            function drawFillPenData():void
            {
                ...
                canvasDraw.graphics.lineStyle(1,xColor);
                canvasDraw.graphics.beginFill(xColor);
                canvasDraw.graphics.drawPath(command,data);
                canvasDraw.graphics.endFill();
                canvasDraw.graphics.moveTo(data[data.length-2],data[data.length-1]);
                canvasDraw.graphics.lineTo(data[0],data[1]);
            ...
            function drawPreviewLine():void
            {
                ...
                dottedLine.ready(canvasDraw.graphics,data[0],data[1]);

                for(var i:uint=2; i<len; i+=2)
                {
                    dottedLine.draw(canvasDraw.graphics,data[i],data[i+1]);
                }
                dottedLine.draw(canvasDraw.graphics,data[0],data[1]);
            ...
            function endFillPenOK():void
            {
                if(checkFillPenUndoReady() === true && command.length > 2)
                {
                    ...
                    canvasDrawLayer.alpha = xAlpha;
                    rDataBuffer.push(["fill5",xColor,xAlpha,xBlendMode,command.concat(),data.concat(),isAirBrushON,airBrushSizeDrawMode]);
            ...
            {
                const target:DisplayObject = e.target as DisplayObject;
                if(isMouseClicked || isQuickSidebarActive ||!target) return;
            ...
            function fillPenMouseDownEvent(e:MouseEvent):void
            {
                ...
                if(sideBar.visible && sideBar.hitTestPoint(mouseX,mouseY))
                {
                    ...
                    if(processColorPickerBoxMouseDown(target))
                    {
                        setFillPenColorPreviewMode();
                        return;
                    }
            ...
            function start():void
            {
                ...
                canvasSizeRect.width = canvasWidth;
                canvasSizeRect.height = canvasHeight;
                ...
                xColor = (isTransparentPenColor) ? canvasBGColor : penColor;
                xAlpha = penAlpha;
                ...
                if(isAirBrushON || eraserAirBrushON)
                {
                    canvasDraw.filters = [];
                }
        ...
        private function cPenTool():Function
        {
            ...
            function checkPenToolUndoReady():void
            {
                ...
                else if(circleRectangleCollision(canvasPanel.mouseX,canvasPanel.mouseY,penCursorSize,0,0,canvasWidth,canvasHeight))
                {
                    canAddUndoData = true;
                }
            }
            ...
            function lineStyleReady(shape:Boolean,size:uint,color:uint,alpha:Number):void
            {
                ...
                if(shape === false)
                {
                    canvasDraw.graphics.lineStyle(size,color);
                }
                else
                {
                    canvasDraw.graphics.lineStyle(size,color,1,false,LineScaleMode.NORMAL,CapsStyle.NONE,JointStyle.BEVEL);
                }
            }
            ...
            function penMove2(mx:Number,my:Number):void
            {
                ...
                if(mouseMovedFlag === false) //움직이기 시작할때 linestyle이랑 moveto넣어줌
                {
                    ...
                    if(xShape)
                    {
                        ...
                        rDataBuffer.push(["lineStyle5",xShape,xSize,xColor,xAlpha,extendedPos.x,extendedPos.y,xBlendMode,false,isSubLayerON,airBrushSizeDrawMode]);
                        penPoints.push(extendedPos.x);
                        penPoints.push(extendedPos.y);
                        canvasDraw.graphics.moveTo(extendedPos.x,extendedPos.y);
                    }
                    else
                    {
                        rDataBuffer.push(["lineStyle5",xShape,xSize,xColor,xAlpha,smoothPos.x+pos05Offset,smoothPos.y+pos05Offset,xBlendMode,false,isSubLayerON,airBrushSizeDrawMode]);
                        penPoints.push(smoothPos.x+pos05Offset);
                        penPoints.push(smoothPos.y+pos05Offset);
                        canvasDraw.graphics.moveTo(smoothPos.x+pos05Offset,smoothPos.y+pos05Offset);
                    }
                }
                ...
                if(mouseMovedFlag)
                {
                    ...
                    canvasDraw.graphics.lineTo(mx,my);
                    ...
                    if(mouseMoveCount >= 100)
                    {
                        ...
                        if(airBrushSizeDrawMode > 0)
                        {
                            ...
                            canvasDraw.filters = [new BlurFilter(blurSize,blurSize,3)];
                            canvasDrawLayerBitmapData.draw(canvasDraw,null,null,"layer");
                            canvasDraw.filters = [];
                        }
                        else
                        {
                        ...
                        canvas2Bitmap.bitmapData = canvasDrawLayerBitmapData;
                        ...
                        if(xShape === true)
                        {
                            rDataBuffer.push(["lineStyle5",xShape,xSize,xColor,xAlpha,prevX,prevY,xBlendMode,false,isSubLayerON,airBrushSizeDrawMode]);
                            penCommand.push(1);
                            penPoints.push(prevX);
                            penPoints.push(prevY);
                            canvasDraw.graphics.moveTo(prevX,prevY);
                        }
                        else
                        {
                            rDataBuffer.push(["lineStyle5",xShape,xSize,xColor,xAlpha,mx,my,xBlendMode,false,isSubLayerON,airBrushSizeDrawMode]);
                            penCommand.push(1);
                            penPoints.push(mx);
                            penPoints.push(my);
                            canvasDraw.graphics.moveTo(mx,my);
                        }
                    }
            ...
            function penToolMouseUpEvent(e:MouseEvent):void
            {
                ...
                if(xShape === true)
                {
                    ...
                    if(mouseMovedFlag === true)
                    {
                        ...
                        if(pointLen >= 4)
                        {
                            ...
                            canvasDraw.graphics.lineTo(extendedPos.x,extendedPos.y);
                        }
                    }
                }
                ...
                {
                    rDataBuffer = [];
                    rDataBuffer.push(["dot4",xShape,xSize,xColor,xAlpha,clickPos.x,clickPos.y,xBlendMode,isSubLayerON,airBrushSizeDrawMode,canvasAnchorPoint.rotation]);
            ...
            return function(penFlag:Boolean):void
            {
                ...
                if(penToolFlag)
                {
                    ...
                    xAlpha = penAlpha;
                    xShape = penIsSquare;
                    xAirBrushON = isAirBrushON;
                    dotflag = true;

                    if(isTransparentPenColor)
                    {
                        xColor = canvasBGColor;
                        xBlendMode = "erase";
                    }
                    else
                    {
        ...
        private function onMouseMoveUpdatePenPreviewCursor(e:MouseEvent):void
        {
        ...
        private function cUpdatePenCursorPosition():Object
        {
            ...
            {
                cursorSize = size*canvasZoomedMultipler;
            }

            function updateZoom(z:Number):void
            {
                if(isNowToolPenOrLine()) cursorSize = penSize*canvasZoomedMultipler;
                else if(isNowTool(TOOL_ERASE)) cursorSize = eraserSize*canvasZoomedMultipler;
                else cursorSize = 0;
            }
            ...
            function check():void
            {
                ...
                if(isSidebarVisible === false && isMouseClickBlocked === false)
                {
		...
		private function initializeCaptureModeTransparentBG():void
        {
        ...
        private function restoreZoomReplayMode():void
        {
            ...
            updateCanvasScale(canvasZoomMultiplerList[rCanvasZoomedIndex],true);
        ...
        private function resetZoomDrawMode():void
        {
            if(canvasZoomedMultipler !== 1.0)
            {
                ...
                const panelLimitedPos:Point = getCanvasBoundLimitPoint(canvasPanel,gcenter.x,gcenter.y,canvasWidth,canvasHeight,canvasAnchorPoint.scaleY,-canvasAnchorPoint.rotation);
        ...
        private function setZoomInButton(zoomInFlag:Boolean,replayMode:Boolean):void
        {
            ...
            else
            {
                ...
                const gcenter:Point = canvasPanel.globalToLocal(new Point(center.x,center.y));
                const gp:Point = canvasPanel.localToGlobal(ZERO_POINT);
                const panelLimitedPos:Point = getCanvasBoundLimitPoint(canvasPanel,gcenter.x,gcenter.y,canvasWidth,canvasHeight,xReg.scaleY,-xReg.rotation);
        ...
        private function onKeyUpLassoTool(e:KeyboardEvent):void
        {
            const keyCode:uint = e.keyCode;
        ...
        private function onKeyDownLassoTool(e:KeyboardEvent):void
        {
            if(isMouseClicked || isRightMouseClicked || isMouseDragging)
            {
                return;
        ...
        private function toggleSideBarPosition():void
        {
        ...
        private function updatePreviewBoxRectPos():void
        {
            ...
            previewBox.updateCursor(gp.x*zoom,gp.y*zoom
                                    ...
                                    ,canvasWidth*zoom,canvasAnchorPoint.rotation);
        }
        ...
        private function updateRGBInfoTextFromColor(color:*):void
        {
            if(color is uint)
            {
                colorPickerBox.setRGBInfoText(getRGBInfoString(color));
            }
            else if(color is Vector.<Number>)
            {
                colorPickerBox.setRGBInfoText(getHSVInfoString(color));
            }
            else
            {
                return;
        ...
        {
            colorPickerBox.updateRGBInfoTextColor(getInvertedColor(color));
        }

        private function initializeColorPickerBoxInfo(color:uint):void
        {
        ...
        private function transferCanvasImageToRefLayer():void
        {
        ...
        private function onMouseOverRefLayerMenuHint(e:MouseEvent):void
        {
            if(!isRefLayerMenuON)
            {
        ...
        private function onMouseOutToolBoxHintOFF(e:MouseEvent):void
        {
            if(isRgbInfoTextFocused) return;
            if(isCanvasZoomHintVisible)
            {
                isCanvasZoomHintVisible = false;
            }
        }
        ...
        private function getToolBoxHint(targetName:String):String
        {
            ...
            switch(targetName)
            {
                ...
                case "toolUndo":
                {
                    str = (isFillPenStarted) ? "Undo":"Undo [z, .]\nRepeat [hold-click]"; break;
                }
        ...
        private function onMouseOverToolBox2HintON(e:MouseEvent):void
        {
            const target:DisplayObject = e.target as DisplayObject;
        ...
        private function onMouseOverToolBoxAppInfoHintON(e:MouseEvent):void
        {
            const target:DisplayObject = e.target as DisplayObject;
            if(!target) return;
            ...
            {
                bottomHint.show(hintStr,target);
            }
        }

        private function changeTopBarIconFromMode(mode:String="draw"):void
        {
            if(isLassoToolON === true || isAboutWindowShowing === true)
            {
                return;
            }
        ...
        private function drawGrid():void
        {
            ...
            if(gridgap*canvasZoomedMultipler < gridgap)
            {
                gridgap = gridgap/canvasZoomedMultipler;
            }
            ...
            if(gridgap !== lastGridGapValue)
            {
                ...
                const gridWidth:Number = canvasWidth;
                const gridHeight:Number = canvasHeight;
            ...
            canvasGrid.graphics.clear();
            canvasGrid.graphics.lineStyle(1/canvasZoomedMultipler,GRID_NORMAL_COLOR,0.5,false);
        ...
        private function cGridFunc():Object
        {
            ...
            function drawDridByValue(mx:Number,initFlag:Boolean):void
            {
                ...
                if(oldValue !== value || initFlag)
                {
                    ...
                    else
                    {
                        ...
                        gridGapValue = value;
                        oldValue = value;
                        setHintONTemp("Grid " + (gridGapValue*GRID_GAP)+"px ("+gridGapValue+"/20)");
            ...
            function start(shortcutKey:Boolean):void
            {
                if(topBar.gridButtonWrapper.visible === false)
                {
                    ...
                    if(gridGapValue > 0) topBar.setGridMoveButtonAlpha(1.0);
        ...
        private function onRightMouseUpRefLayerMenu(e:MouseEvent):void
        {
            ...
            switch(target.name)
            {
                ...
                case "refMoveImageButton":
                {
                    ...
                    && canvasRefLayerBitmap.y !== -canvasRefLayerBitmap.height/2)
                    {
                        ...
                        canvasRefLayer.x = canvasWidth/2;
                        canvasRefLayer.y = canvasHeight/2;
        ...
        private function isRefLayerImageAlreadyCleared():Boolean
        {
        ...
        private function startReflayerClear():void
        {
            setAsTopChild(refLayerMenuBox);
        ...
        private function toggleRefLayerMemoryTraining():void
        {
        ...
        private function startRefLayerImageMirror():void
        {
            ...
            tempBitData.draw(canvasRefLayerBitmapData,flipMat);

            if(canvasRefLayerBitmapData && tempBitData !== canvasRefLayerBitmapData) canvasRefLayerBitmapData.dispose();
            canvasRefLayerBitmapData = tempBitData.clone();
            canvasRefLayerBitmap.bitmapData = canvasRefLayerBitmapData;
            tempBitData.dispose();
            tempBitData = null;
        ...
        private function startRefLayerRotation():void
        {
            const getangle:Function = createAngleUpdaterByMouseDrag(canvasRefLayer);
        ...
        private function startRefLayerImageScale():void
        {
        ...
        private function startRefLayerImageDrag():void
        {
            const getpos:Function = createPosUpdaterByMouseDrag(canvasRefLayerBitmap,
        ...
        private function resetRefLayerMenuOpacity():void
        {
            refLayerLastAlpha = 0.5;
            canvasRefLayer.alpha = 0.5;
            updateRefLayerOpacityCursorPosByValue(0.5);
        ...
        private function startRefLayerOpacityDrag():void
        {
            ...
            startDragInteraction(null,onDragStart,onMouseMoveUpdateopacity,null);
        }
        ...
        private function clearRefLayerImage():void
        {
            canvasRefLayerBitmapData.dispose();
        ...
        private function resetRefLayerImageTransform():void
        {
        ...
        private function updateRefLayerImageTransform(x:Number,y:Number,rotation:Number,scaleX:Number,scaleY:Number):void
        {
            canvasRefLayer.x = canvasWidth/2;
            canvasRefLayer.y = canvasHeight/2;
        ...
        private function transfer2020FileImageToRefLayer(file:File):void
        {
        ...
        private function pasteCanvasImageToRefLayer():void
        {
            if(isDeepUndoEnabled)
            {
                setApplyDeepUndo();
        ...
        private function transferLoadedImageToRefLayer(bmpd:IBitmapDrawable,w:Number,h:Number):void
        {
            ...
            if(canvasRefLayerBitmapData && tmpBMPD !== canvasRefLayerBitmapData) canvasRefLayerBitmapData.dispose();
            canvasRefLayerBitmapData = tmpBMPD.clone();
            canvasRefLayerBitmap.bitmapData = canvasRefLayerBitmapData;
            ...
            const gw:Number = canvasWidth;
            const gh:Number = canvasHeight;
        ...
        private function setEraseAirBrushButtonShortCut():void
        {
            ...
            if(eraserAirBrushON) showMouseHintTemporary("Eraser Air brush ON");
            else showMouseHintTemporary("Eraser Air brush OFF");
        }
        ...
        {
            isAirBrushON = !isAirBrushON;
            setAirBrushCheckBox(isAirBrushON,true);

            if(isAirBrushON) showMouseHintTemporary("Pen Air brush ON");
            else showMouseHintTemporary("Pen Air brush OFF");
        }
        ...
        {
            setSharpLineButton(!isSharpLineON);

            if(isSharpLineON) showMouseHintTemporary("Sharp line ON");
            else showMouseHintTemporary("Sharp line OFF");
        }
        ...
        {
            updateStageBGColor(uiColorSets[index][2]);
            controlBox.changeUIColor(uiColorSets[index][1]);
            colorPickerBox.changeUIColor(uiColorSets[index][1]);
            sideBar.changeUIColor(uiColorSets[index][0]);
            previewBox.chanegStageColor(uiColorSets[index][2]);
            toolBox.changeUIColor(uiToolBoxColorSets[index]);
            toolBox2.changeUIColor(uiToolBoxColorSets[index]);
            fillPenBox.changeBGColor(uiToolBoxColorSets[index]);
            refLayerMenuBox.changeUIColor(uiToolBoxColorSets[index],index === 0);
            lassoMenuBox.changeUIColor(uiToolBoxColorSets[index]);
            numPadBox.changeUIColor(uiToolBoxColorSets[index]);
            topBar.changeUIColor(uiColorSets[index][0],uiColorSets[index][1],uiColorSets[index][4]);
            canvasRotateCursor.changeUIColor(uiColorSets[index][0],uiColorSets[index][1]);
            rReplayTimeBarBox.changeUIColor(uiColorSets[index][0],uiColorSets[index][1],uiToolBoxColorSets[index][4],index);
            checkClipBoardImage();
            canvasInfoBox.changeUIColor(uiColorSets[index][1]);
            setRGBInfoTextColorByColor(colorPickerBox.getRGBInfoBGColor());
            ...
            setResizeButtonColor(uiColorSets[index][3]);

            mouseHint.updateBGColor(hintBGColors[index]);
            bottomHint.setCursorColor(hintHighlightBoxColors[index]);
            fofo.changeColor(uiColorSets[index][1]);
            captureStampFontListBox.changeUIColor(uiColorSets[index][0],uiColorSets[index][1],hintBGColors[index]);

            if(isCanvasWindowON)
            {
                canvasWindow.stage.color = uiColorSets[index][2];
            }

            setPickerBoxTransBGBrightness(index);
        }

        private function addStageInputEvents():void
        {
            ...
            stage.addEventListener(MouseEvent.MOUSE_MOVE,onMouseMoveStage,false,1);
        ...
        private function addInputEventsStageChild():void
        {
            //창을 가운데로 옮김
            ...
            stage.addEventListener(MouseEvent.MOUSE_OUT,onMouseOutGlobalHintOFF);
            ...
            //힌트 보여주는 이벤트
            canvasInfoBox.addEventListener(MouseEvent.MOUSE_OVER,onMouseOverToolBoxAppInfoHintON);
            toolBox.addEventListener(MouseEvent.MOUSE_OVER,onMouseOverToolBoxAppInfoHintON);
            toolBox.addEventListener(MouseEvent.MOUSE_OUT,onMouseOutToolBoxHintOFF);
            toolBox2.addEventListener(MouseEvent.MOUSE_OVER,onMouseOverToolBox2HintON);
            rReplayTimeBarBox.addEventListener(MouseEvent.MOUSE_OVER,onMouseOverTopBarHintON);
            topBar.addEventListener(MouseEvent.MOUSE_OVER,onMouseOverTopBarHintON);
            previewBox.addEventListener(MouseEvent.MOUSE_OVER,onMouseOverPreviewBoxHintON);
            controlBox.addEventListener(MouseEvent.MOUSE_OVER,onMouseOverControlBoxHintON);
            colorPickerBox.addEventListener(MouseEvent.MOUSE_OVER,onMouseOverPickerBoxHintON);
            sideBarScrollBar.addEventListener(MouseEvent.MOUSE_OVER,onMouseOverSideBarScrollHintON);
        ...
        private function onMouseOverPreviewBoxHintON(e:MouseEvent):void
        {
            const target:DisplayObject = e.target as DisplayObject;
            if(!target) return;
            const targetName:String = target.name;
            ...
            bottomHint.show("Canvas navigator",e.currentTarget);
        }
        ...
        private function onMouseOverControlBoxHintON(e:MouseEvent):void
        {
            ...
            if(isFillPenStarted)
            {
                switch(targetName)
                {
                    ...
                    case "alphaButton10":
                    {
                        bottomHint.show(getSizeButtonHint(targetName),target);
                    }
                    return;
            ...
            switch(targetName)
            {
                ...
                case "alphaButton10":
                {
                    bottomHint.show(getOpacityButtonHint(targetName),target);
                }
                return;
        ...
        private function adjustPenAlphaByShortcut(isIncrease:Boolean):void
        {
            var alpha:Number = 0;
            var alphaStr:String = "";
            ...
            function setAlpha(alp:Number,size:uint):void
            {
                ...
                showMouseHintTemporary(alphaStr);
        ...
        private function adjustPenSizeByShortcut(isIncrease:Boolean):void
        {
            ...
            function setSize(index:uint,alpha:Number):void
            {
                ...
                const sizeValueZoomed:Number = sizeValue*canvasZoomedMultipler;

                showMouseHintTemporary(sizeStr);
                setPenSize(index);
                updatePenSizeCursor();

                if(penSizePreviewCursor.visible === true)
                {
            ...
            if(isNowToolPenOrLine() || isNowTool(TOOL_FILL_PEN))
            {
                ...
                if(isAirBrushON && penSize !== airBrushSizeDrawMode)
                {
        ...
        private function checkInvalidKey():void
        {
        ...
        private function getPressedKeyCount():int
        {
            return KEY_BUFFER.length;
        }

        private function isKeyPressed():Boolean
        {
        ...
        private function isTwoKeyPressed():Boolean
        {
            return KEY_BUFFER.length === 2;
        }

        private function isPressdKey(key:int):int
        {
            return KEY_BUFFER.lastIndexOf(key);
        }

        private function getFirstPressedKey():int
        {
            return KEY_BUFFER[0];
        }

        private function getSecondPressedKey():int
        {
            return KEY_BUFFER[1];
        }
        ...
        private function setPenSmoothButton():void
        {
            ...
            const airBrushFlag:Boolean = isNowToolPenOrLine() && isAirBrushON;
            ...
            function setpenSmoothSlideValue():void
            {
                ...
                if(oldValue !== value)
                {
                    ...
                    oldValue = value;
                    bottomHint.show("Pen smoothing "+value + "/"+step,controlBox.penSmoothSliderWapper,true);
                }
            }
        ...
        private function getMergedBitmapdtata(replayMode:Boolean,captureTransparentBG:Boolean,layer1:Boolean,layer2:Boolean,clipRect:Rectangle):BitmapData
        {
            ...
            else
            {
                ...
                xcanvas2 = canvasDrawLayer;
                xBGCOLOR = canvasBGColor;
                alpha = 1.0;
            }
        ...
        private function updateCanvasFlipOnCaptureMode():void
        {
            const xReg:Sprite = (isReplayModeON) ? rCanvasAnchorPoint : canvasAnchorPoint;
        ...
        private function updateCaptureStampButtonAlpha():void
        {
        ...
        private function toggleCaptureStampButton():void
        {
            topBar.capClipBoard.alpha = 1.0;
        ...
        private function setCaptureOFF():void
        {
            ...
            if(isReplayModeON) exitCaptureMode(true,rCanvasAnchorPoint,rCanvasPanel);
            else exitCaptureMode(false,canvasAnchorPoint,canvasPanel);
        }
        ...
        private function setCaptureTransButton(flag:Boolean):void
        {
            ...
            if(isCaptureTransparentBGShowing) setTransBG(isReplayModeON);
            else resetTransBG(isReplayModeON);
        ...
        private function setCanvasResizeButtonVisible(flag:Boolean):void
        {
        ...
        private function updateCanvasResizeButtonVisible(flag:Boolean):void
        {
            ...
            if(flag)
            {
                updateResizeButtonPos(canvasWidth,canvasHeight);
            }

            setCanvasResizeButtonVisible(flag);
        }

        private function updateCanvasResizeButtonVisibleDelay(flag:Boolean):void
        {
            if(flag)
            {
                updateResizeButtonPos(canvasWidth,canvasHeight);
                ...
                {
                    setCanvasResizeButtonVisible(true);
            ...
            else
            {
                ...
                setCanvasResizeButtonVisible(false);
                disableTransparentBGDrawMode();
            }
        }

        private function addInputEventsReplayMode():void
        {
        ...
        private function removeInputEventsReplayMode():void
        {
        ...
        private function removeInputEventsDrawMode():void
        {
        ...
        private function addInputEventsDrawMode():void
        {
        ...
        private function removeInputEventCaptrueMode():void
        {
            ...
            stage.removeEventListener(MouseEvent.MOUSE_OVER,onMouseOverCaptureMode);
        }

        private function addInputEventsCaptrueMode():void
        {
            if(isCaptureModeInputEventsAdded === false)
            {
                ...
                stage.addEventListener(MouseEvent.MOUSE_OVER,onMouseOverCaptureMode,false,-1);
            }
        }

        private function removeInputEventsToolBox2():void
        {
        ...
        private function addInputEventsToolBox2():void
        {
            removeInputEventsDrawMode();
        ...
        {
            if(lassoMenuBox.lassoLayerSwap.alpha < 1.0) return;
            ...
            playLayerSwapFlickEffect(lassoMenuBox.lassoLayerSwap);
        }

        private function setLassoCopyButton():void
        {
            if(isLassoImageCopied) return;
        ...
        {
            var getAngle:Function = createAngleUpdaterByMouseDrag(lassoLayer1);
        ...
        {
            var getMovedPos:Function = createPosUpdaterByMouseDrag(lassoLayer1,canvasAnchorPoint.rotation);
            isMouseDragging = true;
            lassoLayer1Bitmap.smoothing = false;
            lassoLayer2Bitmap.smoothing = false;
        ...
        {
            colorPickerBox.updateCurrentColor(color,getRGBInfoBorderColor(color));
        }

        private function onEnterFrameColorPickerBoxModeBGOFF(e:Event):void
        {
        ...
        private function activeColorPickerModeBG():void
        {
            const color:uint = canvasBGColor;

            isColorPickerModeBG = true;

            moveHSVCursorByColor((isRgbInfoTextHSV) ? HEXtoHSV(color) : color);
            updatePickerCurrentColor(color);
        ...
        private function deactiveColorPickerModeBG():void
        {
        ...
        private function setPenShapeButton(shapeFlag:Boolean):void
        {
            ...
            controlBox.updatePenShapeSet(shapeFlag);
            updatePenSizeCursor();
        }
        ...
        private function startHueColorSelection():void
        {
            ...
            function hueMoveStart(mx:Number):void
            {
                ...
                const color:uint = updatePickerBoxInfoColor(hueValue,hsvColorData[1],hsvColorData[2]);
        ...
        private function tryDisableIME():void
        {
        ...
        private function closeAboutWindow():void
        {
        ...
        private function onAboutWindowMouseDown(e:MouseEvent):void
        {
            const targetName:String = e.target.name;
        ...
        private function clearDataResetVars():void
        {
            ...
            rLastCanvasBGColor = canvasBGColor;
            rMirrorON = false;
        ...
        private function setReRecordCopyCanvas():void
        {
            ...
            rCanvasDrawBitmapData.fillRect(new Rectangle(0,0,rCanvasDrawBitmapData.width,rCanvasDrawBitmapData.height),0);

            if(canvasLayer1BitmapData && canvasLayer1BitmapData !== rCanvasLayer1BitmapData) canvasLayer1BitmapData.dispose();
            canvasLayer1BitmapData = rCanvasLayer1BitmapData.clone();
            canvasLayer1Bitmap.bitmapData = canvasLayer1BitmapData;

            if(canvasLayer2BitmapData && canvasLayer2BitmapData !== rCanvasLayer2BitmapData) canvasLayer2BitmapData.dispose();
        ...
        {
            startPressHoldKey((!keyFlag)?topBar.newFileButton:null,"Creating new file.. ",null,clearData,null);
        }

        private function executeClick(targetName:String):void
        {
            ...
            function onMouseUp(e:MouseEvent):void
            {
                ...
                if(targetName === upTargetName)
                {
                    switch(upTargetName)
                    {
                        ...
                        case "capRotate":
                        {
                            setCaptureRotateButton(++captureCanvasRotationStep);
                        }
                        break;

                        case "capTrans":
                        {
                            setCaptureTransButton(!isCaptureTransparentBGShowing);
                        }
                        break;
                        ...
                        case "capFlip":
                        {
                            setCaptrueFlipButton(!isCaptureCanvasFlipped);
                        }
                        break;
                        ...
                        {
                            setLayerSwapButton();
                            setHintONTemp(getCanvasLayerSwappedHintString());
                        }
                        break;
        ...
        private function deleteReplayFrontData():void
        {
            ...
            createFirstReplayImageFile(rCanvasLayer1BitmapData,rCanvasLayer2BitmapData,RCANVAS_BG_COLOR);

            const fs:FileStream = new FileStream();

            if(rDataReadingFlag === true)
            {
                //repfile 초기화
            ...
            else if(rDataReadingFlag === false)
            {
        ...
        private function initializeReplayDataFile(overWrite:Boolean = false):void //기본 리플레이 파일 만들어줌
        {
            ...
            if(replayDataFilePath.exists === false || overWrite === true)
            {
                ...
                replayCacheImageFolderPath.createDirectory();
                createFirstReplayImageFile(canvasLayer1BitmapData,canvasLayer2BitmapData,canvasBGColor);
            }
        }
        ...
        private function drawFirstJumpImage():void
        {
            ...
            if(rCanvasLayer1BitmapData && bmpd !== rCanvasLayer1BitmapData) rCanvasLayer1BitmapData.dispose();
            rCanvasLayer1BitmapData = bmpd.clone();
            rCanvasLayer1Bitmap.bitmapData = rCanvasLayer1BitmapData;

            if(rCanvasLayer2BitmapData && bmpd1 !== rCanvasLayer2BitmapData) rCanvasLayer2BitmapData.dispose();
            rCanvasLayer2BitmapData = bmpd1.clone();
        ...
        private function createFirstReplayImageFile(bmpd:BitmapData,bmpd1:BitmapData,bgColor:uint):void
        {
            const fs:FileStream = new FileStream();
            var ba:ByteArray = new ByteArray();
            ...
            if(rFirstImageLayer1BitmapData && rFirstImageLayer1BitmapData !== bmpd) rFirstImageLayer1BitmapData.dispose();
            rFirstImageLayer1BitmapData = bmpd.clone();
            ...
            if(rFirstImageLayer2BitmapData && rFirstImageLayer2BitmapData !== bmpd1) rFirstImageLayer2BitmapData.dispose();
            rFirstImageLayer2BitmapData = bmpd1.clone();
        ...
        private function cancelReplayRestartTimer():void
        {
            removeTimer("rReplayRestartTimer");
        ...
        private function setRestartTimer():void
        {
            if(isReplayRepeatON)
            {
                ...
                addTimerByName("rReplayRestartTimer",1.0,true,function():Boolean
                {
        ...
        private function moveImageReplayMode(x:Number,y:Number,layer1:Boolean,layer2:Boolean):void
        {
            ...
            if(layer1)
            {
                tempBitData.draw(rCanvasLayer1BitmapData,movedMat);
                if(rCanvasLayer1BitmapData && tempBitData !== rCanvasLayer1BitmapData) rCanvasLayer1BitmapData.dispose();
                rCanvasLayer1BitmapData = tempBitData.clone();
                rCanvasLayer1Bitmap.bitmapData = rCanvasLayer1BitmapData;
            }
            ...
            if(layer2)
            {
                ...
                tempBitData.draw(rCanvasLayer2BitmapData,movedMat);

                if(rCanvasLayer2BitmapData && tempBitData !== rCanvasLayer2BitmapData) rCanvasLayer2BitmapData.dispose();
                rCanvasLayer2BitmapData = tempBitData.clone();
                rCanvasLayer2Bitmap.bitmapData = rCanvasLayer2BitmapData;
            }
            tempBitData.dispose();
            tempBitData = null;
        ...
        private function replayLineStyleReady(shape:Boolean,size:uint,color:uint,alpha:Number):void
        {
            ...
            if(shape)
            {
                rCanvasDraw.graphics.lineStyle(size,color,1, false,LineScaleMode.NORMAL,CapsStyle.SQUARE,JointStyle.ROUND);
            }
            else
            {
                rCanvasDraw.graphics.lineStyle(size,color);
            }
        }
        ...
        private function replayLineStyleReady2(shape:Boolean,size:uint,color:uint,alpha:Number):void
        {
            ...
            if(shape)
            {
                rCanvasDraw.graphics.lineStyle(size,color,1, false,LineScaleMode.NORMAL,CapsStyle.SQUARE,JointStyle.BEVEL);
            }
            else
            {
                rCanvasDraw.graphics.lineStyle(size,color);
        ...
        private function replayLineStyleReady3(shape:Boolean,size:uint,color:uint,alpha:Number):void
        {
            ...
            if(shape)
            {
                rCanvasDraw.graphics.lineStyle(size,color,1,false,LineScaleMode.NORMAL,CapsStyle.NONE,JointStyle.BEVEL);
            }
            else
            {
                rCanvasDraw.graphics.lineStyle(size,color);
        ...
        private function mirrorCanvasReplayMode():void
        {
            ...
            mirrorBMPD.draw(rCanvasLayer1BitmapData,flipMat);

            if(rCanvasLayer1BitmapData && mirrorBMPD !== rCanvasLayer1BitmapData) rCanvasLayer1BitmapData.dispose();
            rCanvasLayer1BitmapData = mirrorBMPD.clone();
            rCanvasLayer1Bitmap.bitmapData = rCanvasLayer1BitmapData;

            mirrorBMPD.fillRect(new Rectangle(0,0,RCANVAS_WIDTH,RCANVAS_HEIGHT),0);
            mirrorBMPD.draw(rCanvasLayer2BitmapData,flipMat);

            if(rCanvasLayer2BitmapData && mirrorBMPD !== rCanvasLayer2BitmapData) rCanvasLayer2BitmapData.dispose();
            rCanvasLayer2BitmapData = mirrorBMPD.clone();
            rCanvasLayer2Bitmap.bitmapData = rCanvasLayer2BitmapData;
        ...
        private function cReplayDrawCommands():Object
        {
            ...
            function checkSubLayer(subLayerFlag:Boolean):void
            {
                if(subLayerFlag)
                {
                    ...
                    if(rLastSubLayerFlag !== subLayerFlag)
                    {
                        selectReplaySubLayer(subLayerFlag);
                    }
                }
                else if(rLastSubLayerFlag)
                {
            ...
            function lineStyle5(data:Array):void
            {
                ...
                if(fillpen)
                {
                    ...
                    rCanvasDraw.graphics.beginFill(color);
                    rCanvasDraw.graphics.moveTo(startX,startY);
                    rCanvasDrawLayer.alpha = alpha;
                }
                else
                {
            ...
            function lineTo(data:Array):void
            {
                ...
                rCanvasDraw.graphics.lineTo(x,y);
                setRCursorPos(x,y);
            }
            ...
            function sqline(data:Array):void
            {
                ...
                rCanvasDrawBitmapData.dispose();
                rCanvasDrawBitmapData = new BitmapData(RCANVAS_WIDTH,RCANVAS_HEIGHT,true,0);
                rCanvasDraw.graphics.clear();

                updateLineStyleBackup(alpha,blendMode);
                rCanvasDrawLayer.alpha = alpha;
                rCanvasDraw.graphics.lineStyle(size,color,1,false,LineScaleMode.NORMAL,CapsStyle.SQUARE,JointStyle.ROUND);
                rCanvasDraw.graphics.drawPath(command,xyData);
            ...
            function fill5(data:Array):void
            {
                ...
                rCanvasDrawLayer.alpha = alpha;
                rCanvasDraw.graphics.clear();
                rCanvasDraw.graphics.lineStyle(1,color);
                rCanvasDraw.graphics.beginFill(color);
                rCanvasDraw.graphics.drawPath(command,xyData);
                setRCursorPos(xyData[xyData.length-2],xyData[xyData.length-1]);
                resetRCanvasDrawLayerCliprect2();
            ...
            function fill2(data:Array):void
            {
                ...
                rCanvasDraw.graphics.lineStyle(1,color);
                rCanvasDraw.graphics.beginFill(color);
                rCanvasDraw.graphics.moveTo(arr[0],arr[1]);

                for(var i:uint = 2;i<len;i+=2)
                {
                    rCanvasDraw.graphics.lineTo(arr[i],arr[i+1]);
                }

                rCanvasDraw.graphics.endFill();
            ...
            function dot4(data:Array):void
            {
                ...
                if(shape)
                {
                    ...
                    rCanvasDraw.graphics.drawPath(cmd,pos);

                    point = null;
                }
                else
                {
                    rCanvasDraw.graphics.drawCircle(startX,startY,size/2);
                }
            ...
            function dot2(data:Array):void
            {
                ...
                if(shape) rCanvasDraw.graphics.drawRect(startX-size/2,startY-size/2,size,size);
                else rCanvasDraw.graphics.drawCircle(startX,startY,size/2);
                rCanvasDraw.graphics.endFill();
            ...
            function line3(data:Array):void
            {
                ...
                rCanvasDraw.graphics.moveTo(startX,startY);
                rCanvasDraw.graphics.lineTo(endX,endY);
            ...
            function line(data:Array):void
            {
                ...
                if(shape) rCanvasDraw.graphics.lineStyle(size,color,1, false,LineScaleMode.NORMAL,CapsStyle.SQUARE,JointStyle.ROUND);
                else rCanvasDraw.graphics.lineStyle(size,color);
            ...
            function drawDone5(data:Array):void
            {
                ...
                if(subLayer)
                {
                    rCanvasLayer2BitmapData.draw(rCanvasDrawLayerBitmap,null,canvasAlpha,lineStyleData[1],rcanvas2ClipRect2);
                    rCanvasLayer2Bitmap.bitmapData = rCanvasLayer2BitmapData;
                }
                else
                {
                    rCanvasLayer1BitmapData.draw(rCanvasDrawLayerBitmap,null,canvasAlpha,lineStyleData[1],rcanvas2ClipRect2);
                    rCanvasLayer1Bitmap.bitmapData = rCanvasLayer1BitmapData;
                }

                rCanvasDrawBitmapData.fillRect(rcanvas2ClipRect2,0);
                rCanvasDraw.graphics.clear();
            }
            ...
            function drawDone3(data:Array):void
            {
                ...
                if(subLayer)
                {
                    rCanvasLayer2BitmapData.draw(rCanvasDrawLayerBitmap,null,canvasAlpha,lineStyleData[1],rcanvas2ClipRect);
                    rCanvasLayer2Bitmap.bitmapData = rCanvasLayer2BitmapData;
                }
                else
                {
                    rCanvasLayer1BitmapData.draw(rCanvasDrawLayerBitmap,null,canvasAlpha,lineStyleData[1],rcanvas2ClipRect);
                    rCanvasLayer1Bitmap.bitmapData = rCanvasLayer1BitmapData;
                }
            ...
            function drawDone(data:Array):void
            {
                ...
                if(subLayer)
                {
                    ...
                    subLayerBmpd.draw(rCanvasDrawLayerBitmap,null,canvasAlpha);
                    subLayerBmpd.draw(rCanvasLayer1Bitmap);

                    if(rCanvasLayer1BitmapData && subLayerBmpd !== rCanvasLayer1BitmapData) rCanvasLayer1BitmapData.dispose();
                    rCanvasLayer1BitmapData = subLayerBmpd.clone();
                    rCanvasLayer1Bitmap.bitmapData = rCanvasLayer1BitmapData;
        ...
        private function cReplayDraw():Function
        {
            ...
            function readyToReadRData(jumpFlag:int):void
            {
                ...
                if(rData.length > 0)
                {
                    ...
                    replayDrawCommands.ready(rData[rDataIndex]);
                }
                else
                {
                    replayDrawCommands.reset();
            ...
            function drawRData(len:Number,jumpFlag:int):void
            {
                ...
                {
                    if(replayDrawCommands.isIndexBiggerData())
                    {
                        rDataIndex++;
            ...
            return function(jumpCount:Number,jumpFlag:int,replayModeON:Boolean=true):void
            {
                if(jumpCount > 0)
                {
                    ...
                    readCount = jumpCount;
                    if(!rDataReadingFlag) drawFileData(jumpCount,jumpFlag); //여기서 readcount 깍아주고
        ...
        private function cAutoScroll():Object
        {
            ...
            function check(viewCenterFlag:Boolean):void
            {
                ...
                {
                    globalChecked = true;
                    gp = rCanvasLayer1Bitmap.localToGlobal(ZERO_POINT);
        ...
        private function setReplaySpeedButton():void
        {
            ...
            var timeStr:String = getReplayTotalTime(rReplaySpeedMultipler);
            var oldSpeed:uint;
        ...
        {
            if(rReplayImageCacheState === REPLAY_IMAGE_CAHCHE_PROCESSING || isInSaveProgress || isReplayStarted)
            {
        ...
        private function jumpFrame(frame:Number,jumpflag:int):void //jumpp
        {
            ...
            if(updateRCavanvasImageFlag > 0 || frame < rNowFrame)
            {
                ...
                {
                    const file:File = replayCacheImageFolderPath.resolvePath(index+"");
                    const fs:FileStream = new FileStream();
        ...
        private function setJumpFrameButton():void
        {
            ...
            if(totalF === 0 || rReplayImageCacheState > REPLAY_IMAGE_CAHCHE_COMPLETE)
            {
                return;
            }
            ...
            function replayTimeMouseUpEvent(e:MouseEvent):void
            {
                ...
                if(replayStartONSave && !isReplayFinished)
                {
                    startReplay();
                }
        ...
        private function startReplay():void
        {
            ...
            if(isReplayFinishedWithFiwWindow === true)
            {
                ...
                updateCanvasScale(rCanvasZoomedMultiplier,true);
            }
        ...
        private function executeToolBoxClick(targetName:String):void
        {
            function onMouseUpToolBox(e:MouseEvent):void
            {
                ...
                switch(upTargetName)
                {
                    ...
                    case "toolRefLayer":
                    {
                        if(isQuickSidebarActive) _closeQuickSidebar();
        ...
        private function updateCanvasBGColor(xCanvas:Sprite,w:Number,h:Number,color:uint):void
        {
        ...
        private function updateCanvasBGColorReplayMode(color:uint):void
        {
            RCANVAS_BG_COLOR = color;
            updateCanvasBGColor(rCanvasPanel,RCANVAS_WIDTH,RCANVAS_HEIGHT,color);
        }

        private function updateCanvasBGColorDrawMode(color:uint):void
        {
            isFileAlreadySaved = false;
            canvasBGColor = color;
            previewBox.changeprevBitmapBGColor(color);
            updateCanvasBGColor(canvasPanel,canvasWidth,canvasHeight,color);
        ...
        {
            removeTimer("mouseHintShowTimer");
            mouseHint.hide();
            stage.removeEventListener(MouseEvent.MOUSE_DOWN,onMouseDownTimerOFF);
            stage.removeEventListener(MouseEvent.RIGHT_MOUSE_DOWN,onMouseDownTimerOFF);
        }

        private function onMouseDownTimerOFF(e:MouseEvent):void
        {
            hideMouseHint();
        }

        private function showMouseHintTemporary(str:String,time:Number=2.5):void
        {
            if(!hasTimer("mouseHintShowTimer"))
            {
                stage.addEventListener(MouseEvent.MOUSE_DOWN,onMouseDownTimerOFF);
                stage.addEventListener(MouseEvent.RIGHT_MOUSE_DOWN,onMouseDownTimerOFF);
            }

            showMouseHint(str);

            addTimerByName("mouseHintShowTimer",time,false,function():void
            {
                hideMouseHint();
            });
        }

        private function showMouseHint(str:String):void
        {
            if(str !== "")
            {
                mouseHint.setHintText(str);
        ...
        //drag load
        private function updateLoadMenuBoxPosCenter():void
        {
        ...
        private function setLoadBoxReady(pleaseWaitFlag:Boolean,reflayer:Boolean):void
        {
            clearKeyBuffer();
        ...
        private function loadImageFromClipBoard(reflayer:Boolean):void
        {
        ...
        {
            if(isFileBrowserOpened || isCaptureModeON || isInSaveProgress !== 0)
            {
                return;
            }
            ...
            if(arguments && arguments.length > 0)
            {
                try
                {
                    ...
                    if(file.exists)
                    {
                        ...
                        if(hasTimer("rReplayRestartTimer"))
                        {
                            cancelReplayRestartTimer();
                        }
        ...
        private function showLoadBoxPreviewImage(file:File,reflayer:Boolean):void
        {
            if(!file)
            {
                return;
        ...
        private function loadImageDragDrop(toRefLayer:Boolean):void
        {
        ...
        private function selectTegakiColorPreset(index:int):void
        {
            ...
            if(mainColor !== colorPickerBox.getRGBInfoBGColor())
            {
                ...
                moveHSVCursorByColor((isRgbInfoTextHSV) ? HEXtoHSV(penColor):penColor);
            }

            if(!isFillPenStarted)
            {
                const bgColor:uint = myPaletteTegakiPreset[index+10];
                if(bgColor !== canvasBGColor)
                {
                    updateCanvasBGColorDrawMode(bgColor);

                    if(isCanvasWindowON)
                    {
                        updateCanvasWindowBGColor(canvasBGColor,canvasWindowLayer1Bitmap.bitmapData);
                    }
        ...
        private function pickColor(pickedColor:uint):void
        {
            ...
            {
                penColor = pickedColor;
                moveHSVCursorByColor((isRgbInfoTextHSV) ? HEXtoHSV(pickedColor) : pickedColor);
                setNowToolForDrawing(false);
            }
            else if(isBackgroundColorMode())
            {
        ...
        private function initializeMyPaletteList():void
        {
            updateHistoryList();
            updateMyPaletteList();
        ...
        private function initializeResizeButtonFamily():void
        {
        ...
        private function generateReplayCacheImage():void //loadrep
        {
            const fs:FileStream = new FileStream();
            ...
            var _tickDraw:Object = replayDrawCommands;
            var data:Array;
            ...
            //첫 이미지 그려줌
            if(rCanvasLayer1BitmapData && rFirstImageLayer1BitmapData !== rCanvasLayer1BitmapData) rCanvasLayer1BitmapData.dispose();
            rCanvasLayer1BitmapData = rFirstImageLayer1BitmapData.clone();
            rCanvasLayer1Bitmap.bitmapData = rCanvasLayer1BitmapData;

            if(rCanvasLayer2BitmapData && rFirstImageLayer2BitmapData !== rCanvasLayer2BitmapData) rCanvasLayer2BitmapData.dispose();
            rCanvasLayer2BitmapData = rFirstImageLayer2BitmapData.clone();
            rCanvasLayer2Bitmap.bitmapData = rCanvasLayer2BitmapData;
            updateCanvasSizeReplayMode(rCanvasLayer1BitmapData.width,rCanvasLayer1BitmapData.height); //크기도 바꿔주고
            ...
            function onFrameEnter(e:Event):void
            {
                while(true)
                {
                    ...
                    if(namojiBytes === 0)
                    {
                        ...
                        deepUndoStartFrame = TOTAL_FRAME;
                    ...
                    if(_rJumpImageCount > REPLAY_MAKE_JUMPIMAGE_INTERVAL)
                    {
                        ...
                        imgData.clear();
                        imgData1.clear();
                        rCanvasLayer1BitmapData.copyPixelsToByteArray(rect,imgData);
                        rCanvasLayer2BitmapData.copyPixelsToByteArray(rect,imgData1);
                        imgData.compress();
                        imgData1.compress();

                        fs2.open(replayCacheImageFolderPath.resolvePath((rJumpImageFrameData.length-1)+""),FileMode.WRITE);
                        //레이어1,레이어2,가로 세로, 배경색, 마지막 바이트 위치, 마지막 프레임 합
                        fs2.writeObject([imgData // 0
                                        ,imgData1
                                        ,rCanvasLayer1BitmapData.width
                                        ,rCanvasLayer1BitmapData.height
                                        ,rLastCanvasBGColor  //4
                                        ,fs.position
                                        ,_frameSum
                                        ,rMirrorON]); //7
                        fs2.close();
        ...
        private function startPngEncodingWorker(bmpd:BitmapData,bg:uint,isCaptureImage:Boolean,isTransBG:Boolean):void
        {
        ...
        private function startUndoDataCompressionWorker(data:ByteArray,data1:ByteArray):void
        {
            setStartWorker(function():void
            {
                workerDataSendCount++;
        ...
        private function saveReplayFile():void
        {
            if(replayDataFilePath.exists)
            {
                ...
                newRectangle = new Rectangle(0,0,canvasWidth,canvasHeight);
        ...
        private function startReplayDataCompressionWorker(dataA:ByteArray,dataA1:ByteArray,dataB:ByteArray,dataB1:ByteArray,dataC:ByteArray,dataD:ByteArray):void
        {
            setStartWorker(function():void
            {
                workerDataSendCount++;
                                        ...
                                        ,dataD:ByteArray):void
        {
            ...
            fs.writeObject(["rFinalImage",dataB,dataB1,canvasWidth,canvasHeight,canvasBGColor]);
            ...
            while(true)
            {
                ...
                if(d[0] === "rFirstImage")
                {
                    if(d[2] is ByteArray === false)
                    {
                        ...
                        createFirstReplayImageFile(rFirstImageLayer1BitmapData,null,d[4]); //0.cache 파일 갱신
                    }
                    else
                    {
                        ba = d[1] as ByteArray;
                        ...
                        createFirstReplayImageFile(rFirstImageLayer1BitmapData,rFirstImageLayer2BitmapData,d[5]); //0.cache 파일 갱신
                    }
                }
        ...
        private function loadFileAfter(fileName:String,filePath:String, width:uint,height:uint,imageData:IBitmapDrawable,imageData1:IBitmapDrawable,imageOnlyFlag:Boolean,newBG:uint):void
        {
            ...
            if(isFillPenStarted) fillPenTool.cancel();

            tmpBMPD.draw(imageData,scaleMat,null,null,null,true);

            if(canvasLayer1BitmapData && tmpBMPD !== canvasLayer1BitmapData) canvasLayer1BitmapData.dispose();
            canvasLayer1BitmapData = tmpBMPD.clone();
            canvasLayer1Bitmap.bitmapData = canvasLayer1BitmapData;
            ...
            if(imageData1 !== null)
            {
                ...
                if(canvasLayer2BitmapData && tmpBMPD !== canvasLayer2BitmapData) canvasLayer2BitmapData.dispose();
                canvasLayer2BitmapData = tmpBMPD.clone();
                canvasLayer2Bitmap.bitmapData = canvasLayer2BitmapData;
        ...
        private function loadFile(isTransferRefLayer:Boolean=false):void
        {
            if(isReplayStarted) stopReplay();
            if(isLassoToolON || isFileBrowserOpened || isFillPenStarted || isInSaveProgress)
            {
                return;
            }
        ...
        private function setCaptureUI(flag:Boolean):void
        {
            ...
            if(flag)
            {
                if(replayMode) removeInputEventsReplayMode();
                else removeInputEventsDrawMode();
        ...
        private function setDefaultHintCaptureMode():void
        {
            ...
            if(!topBar.hitTestPoint(mouseX,mouseY) && !captureStampFontListBox.hitTestPoint(mouseX,mouseY))
            {
                if(drawCaptureArea.isFullImageCapture())
                {
                    bottomHint.show(drawCaptureArea.getRotatedRectSizeString()+"\nDraw capture area [click+drag]"+STRING_CAPTURE_OK,null,true);
                }
                else
                {
        ...
        private function onMouseOverCaptureMode(e:MouseEvent):void
        {
            addTimerByName("checkCaptureHintDelay",0.1,false,function():void
            {
                ...
                if(!(topBar.hitTestPoint(mouseX,mouseY) || captureStampFontListBox.hitTestPoint(mouseX,mouseY)))
                {
                    bottomHint.hideHighlightBox();
                }
            });
        }

        private function onRightMouseDownCaptureMode(e:MouseEvent):void
        {
        ...
        private function onMouseDownCaptureMode(e:MouseEvent):void
        {
            const target:DisplayObject = e.target as DisplayObject;
            ...
            if(captureStampFontListBox.visible)
            {
                ...
                else if(targetName.indexOf(captureStampFontListBox.getStampFontButtonName()) !== -1)
                {
                    drawCaptureStamp.changeFont(captureStampFontListBox.getFontName(targetName),true);
                }
                else if(target.parent)
                {
                    if(target.parent.name && target.parent.name.indexOf(captureStampFontListBox.getStampFontButtonName()) !== -1)
                    {
                        drawCaptureStamp.changeFont(captureStampFontListBox.getFontName(target.parent.name),true);
                    }
                }
        ...
        private function onKeyUpCaptureMode(e:KeyboardEvent):void
        {
            checkKeyUp(e.keyCode);
        }

        private function onKeyDownCaptureMode(e:KeyboardEvent):void
        {
            ...
            if(isPressingControl())
            {
                ...
                else if(secondKey === KEY.c || secondKey === KEY.m)
                {
                    executeCaptureFlashEffect();
        ...
        {
            if(rReplayImageCacheState === REPLAY_IMAGE_CAHCHE_PROCESSING || isCaptureModeON) return;
            ...
            if(isReplayModeON)
            {
                xReg = rCanvasAnchorPoint;
                xPanel = rCanvasPanel;
                xZoomed = rCanvasZoomedMultiplier;
                rReplayFOFOCursor.visible = false;
            ...
            captureDragAreaOverlay.visible = true;
        ...
        private function exitCaptureMode(replayMode:Boolean,xReg:Sprite,xPanel:Sprite):void
        {
            ...
            isPenSizeCursorInvisible = false;
            captureDragAreaOverlay.visible = false;
            ...
            if(!rFitZoomedON) updateCanvasScale(data.z,replayMode);
            hideMouseHint();
        ...
        private function cDrawCaptureStamp():Object
        {
            ...
            function getSmallBmpd(clipRect:Rectangle):BitmapData
            {
                ...
                if(fullImageFlag)
                {
                    longEdge = canvasHeight > canvasWidth ? canvasHeight : canvasWidth;
            ...
            function checkPosition(bmpdHeight:Number):void
            {
                ...
                if(drawCaptureArea.isFullImageCapture())
                {
                    offsetX = (isReplayModeON) ? RCANVAS_WIDTH:canvasWidth;
                    offsetY = (isReplayModeON) ? RCANVAS_HEIGHT:canvasHeight;
                }
                else
                {
                    offsetX = rect.width;
            ...
            function getCaptureAreaWidth(rect:Rectangle):Number
            {
                ...
                if(notRotatedFlag)
                {
                    if(drawCaptureArea.isFullImageCapture())
                    {
                        return (isReplayModeON) ? RCANVAS_WIDTH:canvasWidth;
                    }
                    else
                    {
                        return rect.width;
                ...
                else
                {
                    if(drawCaptureArea.isFullImageCapture())
                    {
                        return (isReplayModeON) ? RCANVAS_HEIGHT:canvasHeight;
                    }
                    else
                    {
                        return rect.height;
            ...
            function off():void
            {
                ...
                {
                    canvasPanel.scrollRect = new Rectangle(0,0,canvasWidth,canvasHeight);
                }
        ...
        private function cDrawCaptureArea():Object
        {
            ...
            function captureMouseMoveEvent2(e:MouseEvent):void
            {
                ...
                if(mouseMoved === true)
                {
                    if(resizeFlag)
                    {
                        ...
                        bottomHint.show(getRotatedRectSizeString(),null);
                    }
                    else
                    {
                        rectGhost.x += subX;
            ...
            function drawArea(resizeButtonON:Boolean):void
            {
                ...
                captureDragAreaOverlay.graphics.drawRect(0,0,canvasWidth,rect.y); //위
                captureDragAreaOverlay.graphics.drawRect(0,rect.y,rect.x,rect.height);//왼쪽
                captureDragAreaOverlay.graphics.drawRect(rect.x+rect.width,rect.y,canvasWidth-(rect.x+rect.width),rect.height); //오른쪽
                captureDragAreaOverlay.graphics.drawRect(0,rect.y+rect.height,canvasWidth,canvasHeight-(rect.y+rect.height)); //아래
                captureDragAreaOverlay.graphics.endFill();
                captureDragAreaOverlay.graphics.lineStyle(lineSize,0xFFFFFF,1.0,true);
                captureDragAreaOverlay.graphics.drawRect(rect.x,rect.y,rect.width,rect.height);
        ...
        private function saveFile(asFlag:Boolean,saveFailed:Boolean=false):void
        {
            ...
            if(nextPath === saveFilePath && isFileAlreadySaved && continueFlag && fofoFileExists)
            {
            ...
            if(isLassoToolON || isFillPenStarted || isInSaveProgress)
            {
                return;
            }
            ...
            if(continueFlag)
            {
                if(fofoFileExists === true)
                {
                    ...
                    setSaveProgressON();
                    workerPNGSaveData = null;
                    startPngEncodingWorker(mergedImage.clone(),canvasBGColor,false,false);
                    saveReplayFile();
                    updateWindowTitle();
                    clearKeyBuffer();
        ...
        private function loadUndoData():void
        {
            ...
            else if(legacyReplaytotalFrameSave >= 0)
            {
                undoData.setRFileTotalFrame(legacyReplaytotalFrameSave);
            }
        ...
        private function updateAppWindowSizeInfo():void
        {
        ...
        private function saveAppSatate():void
        {
            updateAppWindowSizeInfo();
            ...
            fs.writeObject({"CANVAS_WIDTH":canvasWidth,
                            "CANVAS_HEIGHT":canvasHeight,
                            "canvasZoomIndex":canvasZoomIndex,
                            "canvasZoomedMultipler":(isCaptureModeON) ? drawModeCanvasStateForSaveAppState.z:canvasZoomedMultipler,
                            ...
                            "isRgbInfoTextHSV":isRgbInfoTextHSV,
                            ...
                            "getFirstRCursorPos.x":replayDrawCommands.getFirstRCursorPos().x,
                            "getFirstRCursorPos.y":replayDrawCommands.getFirstRCursorPos().y,
        ...
        private function loadAppState():void
        {
            const fs:FileStream = new FileStream();
            var arr:Array = [];
            var newRectangle:Rectangle;
            ...
            else
            {
                ...
                rFirstImageLayer1BitmapData = new BitmapData(canvasWidth,canvasHeight,true,0);
                rFirstImageLayer2BitmapData = new BitmapData(canvasWidth,canvasHeight,true,0);
            }
            ...
            if(refLayerImageFilePath.exists)
            {
                ...
                if(canvasRefLayerBitmapData && bmpd !== canvasRefLayerBitmapData) canvasRefLayerBitmapData.dispose();
                canvasRefLayerBitmapData = bmpd.clone();
                canvasRefLayerBitmap.bitmapData = canvasRefLayerBitmapData;
                canvasRefLayerBitmap.smoothing = true;
            ...
            if(appStateFilePath.exists)
            {
                ...
                addTimerByName("loadAppDataDelayTimer",0.15,false,function():void
                {
                    ...
                    moveHSVCursorByColor((isRgbInfoTextHSV) ? HEXtoHSV(d["penColor"]) : d["penColor"]);
                    ...
                    controlBox.updatePenShapeSet(d["penIsSquare"]);
            ...
            else //복원파일이 없을때
            {
                ...
                lastAppWindowSize.x = 680;
                lastAppWindowSize.y = 768;
                stage.nativeWindow.width = lastAppWindowSize.x;
                stage.nativeWindow.height = lastAppWindowSize.y;

                updateCavnvasSizeDrawMode(canvasWidth,canvasHeight,0,0,false);
                updateResizeButtonPos(canvasWidth,canvasHeight);
                ...
                updatePreviewBoxRectPos();
                updateAppWindowSizeInfo();
                canvasInfoBox.init(canvasWidth,canvasHeight,Math.floor(canvasZoomedMultipler*100),canvasAnchorPoint.rotation,false);
        ...
        private function cUpdatePenSizeCursor():Function
        {
            ...
            return function():void
            {
                ...
                const z:Number = canvasZoomedMultipler;
        ...
        private function cDrawDone():Function
        {
            ...
            return function():void
            {
                ...
                if(airBrushSizeDrawMode > 0)
                {
                    const blurSize:Number = getBlurSize(airBrushSizeDrawMode,1.0);
                    canvasDraw.filters = [new BlurFilter(blurSize,blurSize,3)];
                    canvasDrawLayerBitmapData.draw(canvasDraw);
                    canvasDraw.filters = [];
                }
                else
                {
                ...
                if(isNowToolPenOrLine() || isNowTool(TOOL_FILL_PEN))
                {
                    ...
                    if(isSubLayerON) canvasLayer2BitmapData.draw(canvas2Bitmap,null,canvas2Alpha,(isTransparentPenColor) ? "erase":null,canvasDrawLayerClipRect);
                    else            canvasLayer1BitmapData.draw(canvas2Bitmap,null,canvas2Alpha,(isTransparentPenColor) ? "erase":null,canvasDrawLayerClipRect);
                }
                else if(isNowTool(TOOL_ERASE))
                {
                    canvas2Alpha.alphaMultiplier = eraserAlpha;

                    if(isSubLayerON) canvasLayer2BitmapData.draw(canvas2Bitmap,null,canvas2Alpha,"erase",canvasDrawLayerClipRect);
                    else           canvasLayer1BitmapData.draw( canvas2Bitmap,null,canvas2Alpha,"erase",canvasDrawLayerClipRect);
                }

                rDataBuffer.push(["drawDone5",isSubLayerON]);

                if(isSubLayerON) canvasLayer2Bitmap.bitmapData = canvasLayer2BitmapData;
        ...
        private function cLineTool():Function
        {
            ...
            function drawingLine():void //지우개인가 펜인가 구분해서 lineto 실시
            {
                ...
                if(xShape)
                {
                    canvasDraw.graphics.lineStyle(xSize, xColor,1,false,LineScaleMode.NORMAL,CapsStyle.NONE,JointStyle.ROUND);
                }
                else
                {
                    canvasDraw.graphics.lineStyle(xSize, xColor);
                }

                canvasDraw.graphics.moveTo(startPoint.x,startPoint.y);
            ...
            function lineMoveEvent(e:MouseEvent):void
            {
                ...
                const mx:Number = canvasDraw.mouseX;
                const my:Number = canvasDraw.mouseY;
            ...
            return function (lineToolFlag:Boolean):void
            {
                ...
                mouseMovedFlag = false;
                oldX = canvasDraw.mouseX;
                oldY = canvasDraw.mouseY;
                subLayerFlag = isSubLayerON
                ...
                canvasDrawLayerBitmapData = new BitmapData(canvasWidth,canvasHeight,true,0);
        ...
        private function cCanvasRotateTool():Function
        {
            ...
            function rotateToolMoveEvent(e:MouseEvent):void
            {
                ...
                canvasInfoBox.setRotate(Math.abs(xReg.rotation));
            }
            ...
            return function (replayMode:Boolean=false):void
            {
                ...
                getAngle = createAngleUpdaterByMouseDrag(xReg);
        ...
        private function cMoveTool():Function
        {
            ...
            function moveToolOFFEvent(e:MouseEvent):void
            {
                ...
                isPenSizeCursorInvisible = false;
                getMovedPos = null;
                var tempBitData:BitmapData = new BitmapData(canvasWidth,canvasHeight,true,0);
                ...
                if(checkedLayer === 0)
                {
                    if(canvasLayer1Bitmap.visible)
                    {
                        ...
                        tempBitData.draw(canvasLayer1BitmapData,movedMat);

                        if(canvasLayer1BitmapData && tempBitData !== canvasLayer1BitmapData) canvasLayer1BitmapData.dispose();
                        canvasLayer1BitmapData = tempBitData.clone();
                        canvasLayer1Bitmap.bitmapData = canvasLayer1BitmapData;
                    }
                    ...
                    if(canvasLayer2Bitmap.visible)
                    {
                        ...
                        tempBitData.fillRect(new Rectangle(0,0,canvasWidth,canvasHeight),0);
                        tempBitData.draw(canvasLayer2BitmapData,movedMat);

                        if(canvasLayer2BitmapData && tempBitData !== canvasLayer2BitmapData) canvasLayer2BitmapData.dispose();
                        canvasLayer2BitmapData = tempBitData.clone();
                        canvasLayer2Bitmap.bitmapData = canvasLayer2BitmapData;
                    }
                }
            ...
            return function ():void
            {
                ...
                getMovedPos = createPosUpdaterByMouseDrag(canvasLayer1Bitmap,canvasAnchorPoint.rotation);
        ...
        {
            const zoomMin:Number = canvasZoomMultiplerList[0];
            const zoomMax:Number = canvasZoomMultiplerList[canvasZoomMultiplerList.length-1];
            ...
            function zoomToolMouseUpEvent(clickZoomInFlag:Boolean):void
            {
                ...
                if(gridGapValue > 0 && oldZoom !== canvasZoomedMultipler)
                {
                    drawGrid();
                }
            }
            ...
            {
                const newZoom:Number = canvasZoomMultiplerList[index];

                updateCanvasScale(newZoom,false);

                showMouseHint(Math.floor(newZoom*100)+"%");
                setFixedToolTipPos();
            }
            ...
            function zoomToolMouseMoveEvent2(dist:Number):void
            {
                ...
                if(canvasZoomIndex < 0) canvasZoomIndex = 0;
                else if(canvasZoomIndex > canvasZoomMultiplerList.length-1) canvasZoomIndex = canvasZoomMultiplerList.length-1;

                zoomGoArray(canvasZoomIndex);
            }
            ...
            return function():void
            {
                ...
                moveFlag = 0; //1이면 x축
                isPenSizeCursorInvisible = true;
                isCanvasZoomHintVisible = true;
                setOptimizeCanvasMoveON(true);
                oldZoom = canvasZoomedMultipler;
                isMouseDragging = true;

                //클릭한 위치가 캔버스밖을 벗어날경우 줌 기준점을 캔버스 경계선에 닿도록 함
                xCanvas = canvasPanel;
                xRotation= -canvasAnchorPoint.rotation;
                maxWidth = canvasWidth*xZoomed;//줌 배율을 곱해줘야 정확한 값이 나옴. width나 canvasPanel.mouseX는 scale된 값이 아님
                maxHeight = canvasHeight*xZoomed;
                ...
                if(isLassoMenuHiddenTemp === true)
                {
                    gp = lassoLayer1.localToGlobal(ZERO_POINT);
                    setRegPoint(gp.x,gp.y,false);
                }
                else
                {
                    gp = xCanvas.localToGlobal(ZERO_POINT);
                    const panelLimitedPos:Point = getCanvasBoundLimitPoint(xCanvas,xCanvas.mouseX,xCanvas.mouseY,canvasWidth,canvasHeight,xZoomed,xRotation);
                ...
                showMouseHint(Math.floor(oldZoom*100)+"%");
                setFixedToolTipPos();
                mouseHint.show();
        ...
        {
            var tempBitData:BitmapData = new BitmapData(canvasWidth,canvasHeight,true,0);
            var flipMat:Matrix = new Matrix(-1,0,0,1,canvasWidth);

            tempBitData.draw(canvasLayer1BitmapData,flipMat);
            ...
            tempBitData.fillRect(new Rectangle(0,0,canvasWidth,canvasHeight),0);
            tempBitData.draw(canvasLayer2BitmapData,flipMat);
        ...
        private function updateGridMirror(mirrorflag:Boolean):void
        {
            if(mirrorflag)
            {
                canvasGrid.scaleX = -1.0;
                canvasGrid.x = canvasWidth;
            }
            else
            {
                canvasGrid.scaleX = 1;
        ...
        private function mirrorRefLayerImage():void
        {
            canvasRefLayer.scaleX = -canvasRefLayer.scaleX;
            canvasRefLayer.rotation = -canvasRefLayer.rotation;
        }
        ...
        private function updateCanvasSizeReplayMode(w:Number,h:Number,moveX:Number=0,moveY:Number=0,movedFlag:Boolean=false):void
        {
            ...
            rCanvasDrawBitmapData = new BitmapData(w,h,true,0);
            RCANVAS_WIDTH = w;
            RCANVAS_HEIGHT = h;
        ...
        private function updateRefLayerImagePos(w:Number,h:Number,movedFlag:Boolean):void
        {
            ...
            const subW:Number = (canvasWidth-w)/2;
            const subH:Number = (canvasHeight-h)/2;
        ...
        private function updateCavnvasSizeDrawMode(w:Number,h:Number,moveX:Number=0,moveY:Number=0,centerMovedFlag:Boolean=false):void
        {
            ...
            updateCanvasBGColor(canvasPanel,w,h,canvasBGColor);
            ...
            if(centerMovedFlag)
            {
                ...
                canvasAnchorPoint.x -= Math.round(rp.x*canvasZoomedMultipler);
                canvasAnchorPoint.y -= Math.round(rp.y*canvasZoomedMultipler);
            }
            else
            {
        ...
        private function cResizeCanvas():Object
        {
            ...
            function drawRatioSnapGuide(w:Number,h:Number,targetName:String):void
            {
                ...
                function _drawRatioLine(referenceSize:Number,offset:Number):void
                {
                    ...
                    for(var i:uint=0;i<len;i+=2)
                    {
                        ...
                        resizePreviewRatioRect.graphics.lineStyle(3/canvasZoomedMultipler,color,1.0,true,"normal","none");
            ...
            function initResizeVars():void
            {
                if(resizeInitON === false)
                {
                    ...
                    guideLineWidth = 30/canvasZoomedMultipler;
        ...
        private function moveSelectedAreaToLassoBox(replayMode:Boolean,rectArr:Vector.<Number>,points:Array,copyFlag:Boolean,layer1:Boolean,layer2:Boolean):Boolean
        {
            ...
            if(replayMode)
            {
                canvas2FilterBackUp = rCanvasDraw.filters.concat();
                rCanvasDraw.filters = [];
                xCanvas2Draw = rCanvasDraw;
            ...
            {
                canvas2FilterBackUp = canvasDraw.filters.concat();
                canvasDraw.filters = [];
                xCanvas2Draw = canvasDraw;
            ...
            if(!copyFlag)
            {
                ...
                xCanvas2Draw.graphics.beginFill(canvasBGColor);
            ...
            if(layer1)
            {
                ...
                lassoLayer1Bitmap.bitmapData.draw(xCanvas2Draw,null,null,"erase");
            }

            if(layer2)
            {
                lassoLayer2Bitmap.bitmapData = lassoBMPDsub;
                lassoLayer2Bitmap.bitmapData.draw(xCanvas2Draw,null,null,"erase");
            }
            ...
            if(replayMode)
            {
                rCanvasDraw.filters = canvas2FilterBackUp.concat();
            }
            else
            {
                canvasDraw.filters = canvas2FilterBackUp.concat();
            }
        ...
        private function cLassoTool():Object
        {
            ...
            {
                const g:Point = lassoLayer1.localToGlobal(ZERO_POINT);
                const lassoW:Number = (lassoMenu.width > stage.stageWidth)
                                      ? stage.stageWidth : lassoMenu.width;

                lassoMenu.x = Math.floor(g.x-lassoW/2);
                lassoMenu.y = Math.floor(g.y+(((lassoLayer1.height)/2)*canvasZoomedMultipler+20));
            ...
            function lassoDrawMouseUp():void
            {
                ...
                if(lassoRect[2] > canvasWidth) lassoRect[2] = canvasWidth;
                if(lassoRect[3] > canvasHeight) lassoRect[3] = canvasHeight;
            ...
            {
                var mx:Number = canvasDraw.mouseX;
                var my:Number = canvasDraw.mouseY;
            ...
            function start ():void
            {
                if(isLassoToolON === true || isAllLayerInvisible()) return;
                ...
                clickPos.setTo(canvasDraw.mouseX,canvasDraw.mouseY);
                ...
                if(canvasLayer1Bitmap.visible)
                {
                    if(lassoLayer1LastBitmapdata && canvasLayer1BitmapData !== lassoLayer1LastBitmapdata) lassoLayer1LastBitmapdata.dispose();
                    lassoLayer1LastBitmapdata = canvasLayer1BitmapData.clone();
                }
                if(canvasLayer2Bitmap.visible)
                {
                    if(lassoLayer2LastBitmapdata && canvasLayer2BitmapData !== lassoLayer2LastBitmapdata) lassoLayer2LastBitmapdata.dispose();
        ...
        private function cSpuitTool():Function
        {
            ...
            {
                const mid:Number = magSize/(4*canvasZoomedMultipler); //4는 기본 중앙값 magsize/2에서 zoomed나워주고 기본이 2배줌이니까 2로 나눠준값
                ...
                spuitMagMat.scale(2.0*canvasZoomedMultipler,2.0*canvasZoomedMultipler);
                ...
                if(canvasLayer2Bitmap.visible)
                {
                    spuitZoomCursor.spuitZoomBitmap.bitmapData.draw(canvasLayer2Bitmap.bitmapData,spuitMagMat,null,null,spuitMagRect);
                }

                if(canvasLayer1Bitmap.visible)
                {
                    spuitZoomCursor.spuitZoomBitmap.bitmapData.draw(canvasLayer1Bitmap.bitmapData,spuitMagMat,null,null,spuitMagRect);
                }
            }
            ...
            function spuitToolKeyDownEvent(e:KeyboardEvent):void
            {
                ...
                else if(e.keyCode === KEY.space)
                {
                    if(isTransparentPenColor)
                    {
                        ...
                        if(sideBar.visible === false)
                        {
                            showMouseHintTemporary("Current color selected");
                        }
                    }
                    else
                    {
                        ...
                        if(sideBar.visible === false)
                        {
                            showMouseHintTemporary("Transparent color selected");
                        }
                    }
                    cancelSpuitTool(false);
                }
            ...
            {
                return !isNowTool(TOOL_SPUIT) || isReplayModeON || isCaptureModeON || isFileBrowserOpened || isMouseClickBlocked;
            }
            ...
            return function ():void
            {
                ...
                updateEraserButtonPosFromOtherTool("toolSpuit");
                spuitZoomCursor.rotateBitmap(canvasAnchorPoint.rotation);
                canvasRefLayer.visible = false;
                canvasBGShape.graphics.clear();
                canvasBGShape.graphics.beginFill(canvasBGColor);
                canvasBGShape.graphics.drawRect(0,0,canvasWidth,canvasHeight);
            ...
            function setTransparentBGTempOFFEvent(e:MouseEvent):void
            {
                ...
                if(dist >= 10)
                {
                    if(timerActivated)
                    {
                        ...
                        canvasPanel.graphics.beginFill(canvasBGColor);
                        canvasPanel.graphics.drawRect(0,0,canvasWidth,canvasHeight);
        ...
        {
            if(canvasRefLayer.alpha > 0.0) canvasRefLayer.visible = !flag;
            if(gridGapValue > 0)  canvasGrid.visible = !flag;
        }
        ...
        private function cHandTool():Function
        {
            ...
            return function (replayMode:Boolean,fromWheelClick:Boolean):void
            {
                ...
                xReg = (isDrawMode) ? canvasAnchorPoint : rCanvasAnchorPoint;
        ...
        private function updateResizeButtonPos(width:Number,height:Number):void
        {
            ...
            const z:Number = 1/canvasZoomedMultipler;
        ...
        private function drawLassoBoxImageToBitmapData(isTransferRefLayer:Boolean):Array
        {
            ...
            else
            {
                ...
                if(canvasLayer1Bitmap.visible)
                {
                    layer1Bmpd = new BitmapData(canvasWidth,canvasHeight,true,0);
                    layer1Bmpd.draw(lassoLayer1Bitmap,posMatrix);
                }

                if(canvasLayer2Bitmap.visible)
                {
                    layer2Bmpd = new BitmapData(canvasWidth,canvasHeight,true,0);
        ...
        private function lassoCancelBmpd():void
        {
            if(lassoLayer1LastBitmapdata)
            {
                if(canvasLayer1BitmapData && lassoLayer1LastBitmapdata !== canvasLayer1BitmapData) canvasLayer1BitmapData.dispose();
                canvasLayer1BitmapData = lassoLayer1LastBitmapdata.clone();
                canvasLayer1Bitmap.bitmapData = canvasLayer1BitmapData;
            }

            if(lassoLayer2LastBitmapdata)
            {
                if(canvasLayer2BitmapData && lassoLayer2LastBitmapdata !== canvasLayer2BitmapData) canvasLayer2BitmapData.dispose();
                canvasLayer2BitmapData = lassoLayer2LastBitmapdata.clone();
                canvasLayer2Bitmap.bitmapData = canvasLayer2BitmapData;
            }
        ...
        {
            setNowTool(TOOL_ZOOM);
            toolBox.moveToolCursor("zoomInButton",canvasInfoBox);
            setControlBoxInfoOFF();
        }
        ...
        private function selectLassoTool():void
        {
            ...
            updateEraserButtonPosFromOtherTool("toolLasso");
        ...
        private function selectFillPenTool():void
        {
            ...
            updateEraserButtonPosFromOtherTool("toolFillPen");
            toolBox.moveToolCursor("toolFillPen");
            setControlBoxInfoOFF();
        }

        private function updateEraserButtonPosFromOtherTool(toolName:String):void
        {
        ...
        private function cCheckSelectMainDrawTool():Function
        {
            ...
            return function (size:uint,color:uint,alpha:Number,shape:Boolean,penFlag:Boolean,lineFlag:Boolean):void
            {
                ...
                if(lineFlag === false)
                {
                    if(penFlag)
                    {
                        updateEraserButtonPosFromOtherTool("toolPen");
                        toolBox.moveToolCursor("toolPen");
                        setControlBoxInfoOFF();
                    }
                    else
                    {
                        if(lastEraserPosButton) lastEraserPosButton.visible = true;
                ...
                else //선툴을 선택했을때
                {
                    if(penFlag)
                    {
                        updateEraserButtonPosFromOtherTool("toolLine");
                ...
                controlBox.updatePenShapeSet(shape);
                penCursorPosition.check();
            }
        }
        ...
        private function selectEraseTool():void
        {
            ...
            checkSelectMainDrawTool(eraserSize,canvasBGColor,eraserAlpha,eraserIsSquare,false,false);
        }
        ...
        private function updateRefLayerBitmapPos(pos:Point):void
        {
        ...
        private function drawUndoData(redoFlag:Boolean=false):void
        {
            ...
            if(undoRefData[2] !== RCANVAS_WIDTH || undoRefData[3] !== RCANVAS_HEIGHT) updateCanvasSizeReplayMode(undoRefData[2],undoRefData[3],0,0,false);
            if(undoRefData[4] !== RCANVAS_BG_COLOR) updateCanvasBGColorReplayMode(undoRefData[4]);

            rCanvasDraw.graphics.clear();

            if(rCanvasLayer1BitmapData && undoRefData[0] !== rCanvasLayer1BitmapData) rCanvasLayer1BitmapData.dispose();
            rCanvasLayer1BitmapData = undoRefData[0].clone();
            rCanvasLayer1Bitmap.bitmapData = rCanvasLayer1BitmapData;

            if(rCanvasLayer2BitmapData && undoRefData[1] !== rCanvasLayer2BitmapData) rCanvasLayer2BitmapData.dispose();
            rCanvasLayer2BitmapData = undoRefData[1].clone();
            rCanvasLayer2Bitmap.bitmapData = rCanvasLayer2BitmapData;
            ...
            if(rData.length > 0)
            {
                for(var i:int=0; i<=undoIndexSave; i++)
                {
                    ...
                    replayDrawCommands.ready(rData[i]);
            ...
            if(movedRegPos)
            {
                canvasAnchorPoint.x += movedRegPos.x*canvasZoomedMultipler;
                canvasAnchorPoint.y += movedRegPos.y*canvasZoomedMultipler;
                updateRefLayerBitmapPos(movedRegPos);
            }

            if(canvasLayer1BitmapData && rCanvasLayer1BitmapData !== canvasLayer1BitmapData) canvasLayer1BitmapData.dispose();
            canvasLayer1BitmapData = rCanvasLayer1BitmapData.clone();
            canvasLayer1Bitmap.bitmapData = canvasLayer1BitmapData;

            if(canvasLayer2BitmapData && rCanvasLayer2BitmapData !== canvasLayer2BitmapData) canvasLayer2BitmapData.dispose();
            canvasLayer2BitmapData = rCanvasLayer2BitmapData.clone();
            canvasLayer2Bitmap.bitmapData = canvasLayer2BitmapData;

            setRCursorVisibleONUndo(undoDataIndex);
            checkMirrorCanvasReplayMirror();
        ...
        private function undo():void
        {
            ...
            else
            {
                ...
                if(undoDataIndex < -1)
                {
                    ...
                    if(rReplayImageCacheState === REPLAY_IMAGE_CAHCHE_READY || (rReplayImageCacheState === REPLAY_IMAGE_CAHCHE_COMPLETE && undoData.getRFileTotalFrame() > 0))
                    {
                        enableDeepUndo();
                        setRCursorVisibleONFadeOFF();
                    }
        ...
        private function drawReplayImageToDrawModeCanvas():void
        {
            ...
            previewBox.updateImage(canvasLayer1BitmapData,canvasLayer2BitmapData,RCANVAS_BG_COLOR);
        ...
        private function cAddUndoData():Object
        {
            ...
            var undoRefImage:Array = [rFirstImageLayer1BitmapData.clone()
            ...
            {
                undoData.setUndoRefImage(rCanvasLayer1BitmapData.clone()
                                        ,rCanvasLayer2BitmapData.clone()
                                        ,rCanvasLayer1BitmapData.width
                                        ,rCanvasLayer1BitmapData.height
                                        ,RCANVAS_BG_COLOR
            ...
            {
                undoData.setUndoRefImage(canvasLayer1BitmapData.clone()
            ...
            function updateUndoRefImage():void
            {
                ...
                if(undoRefImage[2] !== RCANVAS_WIDTH || undoRefImage[3] !== RCANVAS_HEIGHT) updateCanvasSizeReplayMode(undoRefImage[2],undoRefImage[3],0,0,false);
                if(undoRefImage[4] !== RCANVAS_BG_COLOR) updateCanvasBGColorReplayMode(undoRefImage[4]);

                if(rCanvasLayer1BitmapData && undoRefImage[0] !== rCanvasLayer1BitmapData) rCanvasLayer1BitmapData.dispose();
                rCanvasLayer1BitmapData = undoRefImage[0].clone();

                if(rCanvasLayer2BitmapData && undoRefImage[1] !== rCanvasLayer2BitmapData) rCanvasLayer2BitmapData.dispose();
                rCanvasLayer2BitmapData = undoRefImage[1].clone();

                rCanvasLayer1Bitmap.bitmapData = rCanvasLayer1BitmapData;
                rCanvasLayer2Bitmap.bitmapData = rCanvasLayer2BitmapData;

                replayDrawCommands.ready(rData[0]);
                replayDrawCommands.drawAll();

                if(undoRefImage[0] && undoRefImage[0] !== rCanvasLayer1BitmapData) undoRefImage[0].dispose();
                if(undoRefImage[1] && undoRefImage[1] !== rCanvasLayer2BitmapData) undoRefImage[1].dispose();
                undoRefImage[0] = rCanvasLayer1BitmapData.clone();
                undoRefImage[1] = rCanvasLayer2BitmapData.clone();
                    ...
                    if(oldData.length > 0)
                    {
                        ...
                        if(rReplayImageCacheState === REPLAY_IMAGE_CAHCHE_COMPLETE)
                        {
                            if(rJumpImageCount > REPLAY_MAKE_JUMPIMAGE_INTERVAL)
                            {
                                ...
                                startUndoDataCompressionWorker(imgData,imgData1);
                                ...
                                if(!hasTimer("workerUndoDataTimer"))
                                {
                                    addTimerByName("workerUndoDataTimer",WORKER_WAIT_INTERVAL,true,function():Boolean
                                    {
                                        if(workerUndoData.length > 0)
                                        {
                                            ...
                                            fs.open(replayCacheImageFolderPath.resolvePath((rJumpImageFrameData.length-1)+""),FileMode.WRITE);
        ...
        private function moveHSVCursorByColor(color:*):void
        {
            ...
            if(colorPickerBox.getRGBInfoText().charAt(0) === 'R')
            {
            ...
            colorPickerBox.updateRGBInfoBG(hexColor,getRGBInfoBorderColor(hexColor),myPalettePresetType);
        }
        ...
        private function cDrawDot():Function
        {
            ...
            {
                canvasDraw.graphics.clear();
                canvasDraw.graphics.lineStyle(0,0,0);
                canvasDraw.graphics.beginFill(color);
                ...
                if(shape === true)
                {
                    ...
                    canvasDraw.graphics.drawPath(cmd,pos);
                }
                else
                {
                    canvasDraw.graphics.drawCircle(posX,posY,size/2);
                }
                canvasDraw.graphics.endFill();
            }
        }
        ...
        private function setSideBarRightPosition(ignoreCanvasMove:Boolean):void
        {
            ...
            sideBarScrollPanel.x = 9;
            sideBarScrollPanel.y = scrollSetMovedY;
            previewBox.x = -4;
            previewBox.y = 0;
            canvasInfoBox.setWidth(previewBox.BOX_WIDTH);
            canvasInfoBox.x = previewBox.x-2;
            canvasInfoBox.y = Math.floor(previewBox.y+previewBox.BOX_HEIGHT+6);
            controlBox.x = 39;
            controlBox.y = Math.floor(canvasInfoBox.y+canvasInfoBox.height+7);
            colorPickerBox.x = 39;
            colorPickerBox.y = Math.floor(controlBox.y+controlBox.height+10);
            toolBox.x = -2;
            ...
            if(sideBar.visible)
            {
                if(ignoreCanvasMove === false) canvasAnchorPoint.x -= STAGE_RIGHT_OFFSET;
        ...
        private function initializeAppMenus():void
        {
            ...
            colorPickerBox.rgbInfo.addEventListener(FocusEvent.FOCUS_IN, rgbInfoTextFocusInEvent);
            colorPickerBox.rgbInfo.addEventListener(FocusEvent.FOCUS_OUT, rgbInfoTextFocusOutEvent);
            previewBox.scrollRect = new Rectangle(0,0,previewBox.width,previewBox.height);

            sideBarScrollPanel.addChild(previewBox);
            ...
            sideBarScrollPanel.addChild(controlBox);
            ...
            stage.addChild(mouseHint);
            stage.addChild(bottomHint.getHighlightBox());
            stage.addChild(numPadBox);
            stage.addChild(captureStampFontListBox);
            setAsTopChild(topBar);
        }

        private function initializeReplayCanvas():void
        {
            rCanvasPanel.name = "rcanvasPanel";
            rCanvasAnchorPoint.name = "rregPoint";
            rCanvasLayer1Bitmap.name = "rcanvas1Bitmap";
            ...
            rCanvasPanel.graphics.beginFill(canvasBGColor);
            rCanvasPanel.graphics.drawRect(0,0,canvasWidth,canvasHeight);
            rCanvasPanel.graphics.endFill();

            rCanvasDrawLayer.addChild(rCanvasDrawLayerBitmap);
            rCanvasDrawLayer.addChild(rCanvasDraw);//canvas2에
            rCanvasDrawLayer.blendMode = "layer";//캔버스1이랑 알파 불투명도가 겹치지 않게 layer모드로 해줌

            rCanvasPanel.addChild(rCanvasLayer2Bitmap);//판넬에 canvas11추가
            rCanvasPanel.addChild(rCanvasLayer1Bitmap);//판넬에 canvas1추가
            rCanvasPanel.addChild(rCanvasDrawLayer);//판넬에 canvas2추가
        ...
        private function initializeCanvas():void
        {
            ...
            canvasAnchorPoint.name = "regPoint";
            canvasLayer1Bitmap.name = "canvas1Bitmap";
            canvasLayer2Bitmap.name = "canvas11Bitmap";
            canvasDrawLayer.name = "canvas2";
            canvasDraw.name = "canvas2Draw";
            ...
            updateCanvasBGColorDrawMode(canvasBGColor);
            updateCanvasPanelMask(canvasWidth,canvasHeight);
            ...
            canvasDrawLayer.addChild(canvas2Bitmap);
            canvasDrawLayer.addChild(canvasDraw);
        ...
        private function resetScrollBarX():void
        {
        ...
        private function updateCanvasScale(zoomValue:Number,isReplayMode:Boolean = false):void
        {
            ...
            {
                xReg = canvasAnchorPoint;
                canvasZoomedMultipler = zoomValue;

                if(!isCaptureModeON)
                {
                    penCursorPosition.updateZoom(zoomValue);
                }
            }
            else
            {
            ...
            xReg.scaleX = zoomValue;
            xReg.scaleY = zoomValue;
        ...
        private function onWindowClosingEvent(e:Event):void
        {
            isWindowClosing = true;

            if(isCaptureModeON === true) setCaptureOFF();
            if(isReplayStarted === true) stopReplay();
            if(isLassoToolON) setLassoCancelButton();
        ...
        private function keepBoxInsideStage(ent:DisplayObject):void
        {
        ...
        private function keepCnvasPanelInStage(replayMode:Boolean = false):void
        {
            var xReg:Sprite;
            var xCanvas:Bitmap;
        ...
        private function centerCanvas(isReplayMode:Boolean=false,isCaptureMode:Boolean=false):void
        {
            var xReg:Sprite;
            var xCanvas:Sprite;
            var w:Number;
            ...
            if(isCaptureMode) center = getStageCenterPos(2);
            else if(isReplayMode) center = getStageCenterPos(1);
        ...
        private function clearCanvasReplayMode():void
        {
            ...
            rCanvasLayer1BitmapData.fillRect(rect,0);
            rCanvasLayer2BitmapData.fillRect(rect,0);
            rCanvasDrawBitmapData.fillRect(rect,0);
        }

        private function clearCanvas():void
        {
            const rect:Rectangle = new Rectangle(0,0,canvasWidth,canvasHeight);
        ...
        private function onKeyDownReplayMode(e:KeyboardEvent):void//keydown2
        {
            const firstKey:uint = getFirstPressedKey();
            ...
            {
                checkSubKey(3,false,function(input:int):void
                {
                    if(input === KEY.s)
                    {
                        saveFile(true);
            ...
            else if(isPressingControl())
            {
                checkSubKey(2,false,function(input:int):void
                {
                    ...
                    else if(input === KEY.c || input === KEY.m) enterCaptureMode();
                });
                return;
            }
            ...
            switch(firstKey)
            {
                ...
                case KEY.f2:
                {
                    ...
                    {
                        startPressHoldKey(null,"Creating new file from this image..",setDeleteBarReRecord,setReRecord,setReplayDeleteBarVisibleOFF);
                    }
                }
                break;
                ...
                case KEY.f3:
                {
                    ...
                    {
                        startPressHoldKey(null,"Deleting data..",setDeleteBarDeleteFrontData,deleteReplayFrontData,setReplayDeleteBarVisibleOFF);
                    }
                }
                break;
                ...
                case KEY.f4:
                {
                    ...
                    {
                        startPressHoldKey(null,"Deleting data.. ",setDeleteBarSuperUndo,superUndo,setReplayDeleteBarVisibleOFF);
                    }
                }
                break;
        ...
        private function onKeyUpDrawMode(e:KeyboardEvent):void //keyup1
        {
            const keyCode:uint = e.keyCode;
        ...
        private function checkSubKey(expectedLength:uint,updateFlag:Boolean,callback:Function):Boolean
        {
        ...
        private function onKeyDownDrawMode(e:KeyboardEvent):void
        {
            ...
            if(isTwoKeyPressed())
            {
                ...
                if(firstKey === KEY.d || firstKey === KEY.j)
                {
                    ...
                    {
                        if(isQuickSidebarActive === false) setQuickSidebarON(true);
                        return;
                    }
                    else if(checkPenOptionsKeyDown(secondKey))
                    {
        ...
        private function checkEtcKeyDown(keyCode:int):Boolean
        {
            switch(keyCode)
            {
                ...
                case KEY.n9:
                {
                    ...
                    {
                        selectSubLayer(false,canvasLayer2Bitmap.visible);
                    }
                    ...
                    showMouseHintTemporary("Layer 1 selected");
                }
                return true;
                ...
                case KEY.n0:
                {
                    ...
                    {
                        selectSubLayer(true,canvasLayer1Bitmap.visible);
                    }
                    ...
                    showMouseHintTemporary("Layer 2 selected");
                }
                return true;
        ...
        private function checkToolKeyDown(keyCode:int):void
        {
            ...
            switch (keyCode)
            {
                ...
                case KEY.backspace:
                {
                    if(topBar.newFileButton.alpha === 1.0 && !isInSaveProgress)
                    {
                        setClearData(true);
                    }
                }
        ...
        private function onWindowDeactivate(e:Event):void
        {
            ...
            if(appResetFlag === false)
            {
                ...
                && !isInSaveProgress && !isFileBrowserOpened && !isLoadPendingAfterSaving && !isUpdatePendingAfterSaving && !loadMenuBox.visible)))
                {
        ...
        private function onMouseDownToolBox2(e:MouseEvent):void
        {
            const target:DisplayObject = e.target as DisplayObject;
            if(!target)
            {
        ...
        //툴메뉴 오른쪽 클릭 땠을때
        private function onRightMouseUpToolBox2(e:MouseEvent):void
        {
            isPenSizeCursorInvisible = false;
        ...
        private function setScrollBarMoveButton(deltaY:Number=0.0):void
        {
            ...
            if(deltaY === 0.0) isMouseDragging = true;
            bottomHint.hide();
        ...
        private function processToolBoxMouseDown(target:DisplayObject):Boolean
        {
        ...
        private function startCanvasResizing(targetName:String):void
        {
            isPenSizeCursorInvisible = true;
            penSizePreviewCursor.visible = false;
            showMouseHint(canvasWidth+" x "+canvasHeight);
        ...
        private function updateReplayCursorScale(zoom:Number):void
        {
        ...
        private function disableDeepUndo():void
        {
            isDeepUndoEnabled = false;
        ...
        private function enableDeepUndo():void
        {
        ...
        private function startGeneratingReplayCacheImage():void
        {
        ...
        private function syncReplayCanvasImageWithDrawMode():void
        {
            rCanvasDraw.graphics.clear();

            if(rCanvasLayer1BitmapData && canvasLayer1BitmapData !== rCanvasLayer1BitmapData) rCanvasLayer1BitmapData.dispose();
            rCanvasLayer1BitmapData = canvasLayer1BitmapData.clone();
            rCanvasLayer1Bitmap.bitmapData = rCanvasLayer1BitmapData;

            if(rCanvasLayer2BitmapData && canvasLayer2BitmapData !== rCanvasLayer2BitmapData) rCanvasLayer2BitmapData.dispose();
            rCanvasLayer2BitmapData = canvasLayer2BitmapData.clone();
            rCanvasLayer2Bitmap.bitmapData = rCanvasLayer2BitmapData;

            updateCanvasSizeReplayMode(canvasLayer1Bitmap.width,canvasLayer1Bitmap.height);
            updateCanvasBGColorReplayMode(canvasBGColor);
        }

        private function updateReplayTimeBarFromDrawMode():void
        {
            const totalFrame:Number = TOTAL_FRAME;
        ...
        private function exitReplayMode():void
        {
            ...
            updatePreviewBoxRectPos();
            if(isTransparentPenColor) selectTransparentColorButton();
            else deactiveColorPickerModeBG();
            updatePenSizeCursor();
            penCursorPosition.check();
            changeTopBarIconFromMode("draw");
            canvasInfoBox.setZoom(canvasZoomedMultipler);
            updateReplayCursorScale(canvasZoomedMultipler);
        ...
        private function enterReplayMode():void
        {
            if(rReplayImageCacheState === REPLAY_IMAGE_CAHCHE_PROCESSING)
            {
                return;
            ...
            if(rReplayImageCacheState === REPLAY_IMAGE_CAHCHE_COMPLETE)
            {
                if(canvasWidth === RCANVAS_WIDTH && canvasHeight === RCANVAS_HEIGHT)
                {
            ...
            updateReplayBarPos(stage.stageWidth);
            autoScroll.updateRCanvasBounds();
            updateReplayCursorScale(rCanvasZoomedMultiplier);
        ...
        private function onMouseDownReplayMode(e:MouseEvent):void //repdown1
        {
            const target:DisplayObject = e.target as DisplayObject;
            ...
            switch(targetName)
            {
                case "reRecordingButton":
                {
                    startPressHoldKey(topBar.reRecordingButton,"Creating new file from this image.. ",setDeleteBarReRecord,setReRecord,setReplayDeleteBarVisibleOFF);
                }
                break;
                ...
                case "cutPrevDataButton":
                {
                    if(topBar.cutPrevDataButton.alpha === 1.0)
                    {
                        startPressHoldKey(topBar.cutPrevDataButton,"Deleting data.. ",setDeleteBarDeleteFrontData,deleteReplayFrontData,setReplayDeleteBarVisibleOFF);
                    }
                }
                break;
                ...
                case "superUndoButton":
                {
                    if(topBar.superUndoButton.alpha === 1.0)
                    {
                        startPressHoldKey(topBar.superUndoButton,"Deleting data.. ",setDeleteBarSuperUndo,superUndo,setReplayDeleteBarVisibleOFF);
                    }
                }
                break;
        ...
        private function onMouseUpDrawMode(e:MouseEvent):void //mouseup1
        {
        ...
        private function onRightMouseDownReplayMode(e:MouseEvent):void
        {
        ...
        private function onRightMouseDownDrawMode(e:MouseEvent):void //rdown1
        {
            if(isMouseClicked || isPressingControl() || isQuickSidebarActive
            || isFillPenStarted || isNowTool(TOOL_SPUIT) || (isRefLayerMenuON && refLayerMenuBox.hitTestPoint(mouseX,mouseY))
            ...
            switch(targetName)
            {
                ...
                default:
                {
                    ...
                    {
                        if(isToolBox2ON && !isDeepUndoEnabled)
                        {
                            closeToolBox2();
                        }
                        else
        ...
        private function checkControlBoxButtons(target:DisplayObject):Boolean
        {
            ...
            switch(targetName)
            {
                ...
                case "layerSwapButton":
                {
                    if(isToolBox2ON || target.alpha < 1.0)
                    {
                        return true;
                    }
                    executeClick(targetName);
                ...
                case "airBrushText":
                {
                    if(controlBox.airBrushButtonWrapper.alpha === 1.0)
                    {
                        ...
                        if(isNowToolPenOrLine() || isNowTool(TOOL_FILL_PEN))
                        {
                            setPenAirBrushButton(!isAirBrushON);
                        }
                        else if(isNowTool(TOOL_ERASE))
                        {
        ...
        private function executeColorPickerBoxClick(targetName:String):void
        {
            function onMouseUpColorPickerBox(e:MouseEvent):void
            {
                ...
                if(targetName === upTargetName)
                {
                    switch(upTargetName)
                    {
                        case "currentColor":
                        {
                            updateCurrentColorPickerBox(isColorPickerModeBG);
                            setNowToolForDrawing(false);
                        }
                        break;
                        ...
                        {
                            if(colorPickerBox.transColorButton.alpha === 1.0)
                            {
                                selectTransparentColorButton();
                                setNowToolForDrawing(false);
                            }
        ...
        private function processColorPickerBoxMouseDown(target:DisplayObject):Boolean
        {
            if(isToolBox2ON || (isKeyPressed()
                                && !isNowToolPenOrLine()
            ...
            switch(targetName)
            {
                case "scratchPad":
                {
                    colorPickerBox.scratchPad.drawReady(penSize,penColor,penAlpha,penIsSquare,pickColor,getColorDifferenceForHuman);
                }
                return true;
                ...
                case "myPaletteBox":
                {
                    ...
                    {
                        executeColorPickerBoxClick(targetName);
                    }
                }
                return true;
        ...
        private function onRightMouseDownLassoTool(e:MouseEvent):void
        {
            if(!isLassoToolON)
            {
                return;
        ...
        private function onRightMouseUpLassoTool(e:MouseEvent):void
        {
            if(!isLassoToolON || isMouseClicked)
            {
                return;
            }
        ...
        private function onMouseUpLassoTool(e:MouseEvent):void
        {
        ...
        private function onMouseDownLassoTool(e:MouseEvent):void
        {
            if(isRightMouseClicked)
            {
                return;
        ...
        private function onMouseDownDrawMode(e:MouseEvent):void
        {
            ...
            if(sideBar.visible)
            {
                if(sideBarScrollPanel.hitTestPoint(mouseX,mouseY))
                {
                    ...
                    else if(processColorPickerBoxMouseDown(target) && !isKeyPressed())
                    {
                        return;
                    }
                    ...
                    else if(toolBox.alpha === 1.0 && target.alpha === 1.0 && processToolBoxMouseDown(target))
                    {
                        return;
                    }
                }
            ...
            switch (targetName)
            {
                ...
                case "newWindowCloseButton":
                {
                    if(isToolBox2ON || isKeyPressed() || e.target.alpha < 1.0)
                    {
                        return;
                    }
