package Modules
{
    ...
    public class UndoController
    {
        ...
        private static function syncNowFrameWithTotalFrame():void
        {
        ...
        public static function addNew():void
        {
            ...
            if (ReplayController.rData.length >= NATIVE_UNDO_LIMIT_COUNT) // 첫번째 이미지는 빼야하니깐 -1로 계산해야함
            {
                ...
                if (oldData.length > 0)
                {
                    ...
                    const lastRDataTotalFrame:Number = ReplayController.getRFileTotalFrame();
                        ...
                        if (dataWriteCount > ReplayController.REPLAY_DISK_CACHE_FRAME_INTERVAL)
                        {
                            ...
                            BackgroundWorkerCoordinator.undoDataQueue.push(
                                    new CacheImageMetaData(
                                        ...
                                        rf.size,
                                        lastRDataTotalFrame,
                                        ReplayController.getRFileTotalFrame(),
                                        data[5]));
